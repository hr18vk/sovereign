...
Sovereign Engine's CRDT core holds a gate-passing floor of **50,736,038 ops/s**, re-measured on the current tree at **68,278,197 ops/s** on a 32-core AWS Graviton (c8g.8xlarge), Go 1.26.1 — the CORE microbench (`TestScalingGate`), NOT the ingest rate (5.7M-6.0M deltas/sec @32c — the measured provenance for both layers is in `docs/evidence/`). Every rule
...
Sovereign Engine's write path — `HAMT.Set`, `DeltaCRDTEngine.InsertLocal`,
...
- **Every contended atomic is alone on its line.** `EBRManager.globalEpoch`,
  ...
  global `DeltaCRDTEngine.state` CAS of the earlier design is gone — the hot locus
...
- **The cost of ignoring this is measured.** The legacy single-locus SEC gate
  ...
  **1.1M ops/s at 32 cores (1.6% parallel efficiency)** — a HITM storm. The
  Home-Shard Allocator disperses that locus across 64 lines and restores the
  **50.7M-68.3M ops/s** range. Two atomics on one line is not a sub-microsecond rounding
...
- `recover()` is blind to `SIGSEGV`. Crash isolation is a separate address
  ...
  the crash signal), never a panic boundary. **WAL replay RESTORES the recorded
  dot — it never re-mints** (ADR-0045): each mutation
  is re-applied at its WAL-recorded `(DotNodeID, DotCounter)` via the exact-dot
  route (synthetic-delta `Join`), and the post-checkpoint tail is cut by WAL
  RECORD SEQUENCE, never by a scalar counter. Both historical seed formulas —
  `LamportHigh - len(Mutations)` and `firstMutation.Counter - 1`
  — are refuted: they assume WAL append order reproduces mint order, and two
  concurrent writers can append in the opposite order. Re-minting on the replay
  path double-mints, which is data corruption (measured: 10,800 live dots for
  10,000 keys, permanent divergence).

## 4. Absolute Honesty About Headroom

The cardinal reporting rule:

- A gate that passes by 8.5% is a gate that passed. It is not a "shatter." Cite
  the range — the 32-core figure varies across clean runs (gate floor 50.7M;
  the current tree re-measures up to 68.3M); the 25.1%
