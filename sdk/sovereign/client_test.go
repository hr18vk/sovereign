...
import (
	...
	"github.com/hr18vk/sovereign/sdk/sovereign"
)
...
// SDK dials). It mirrors the gossip_test.go harness shape (dev CA +
...
type testNode struct {
	...
	// optional /metrics route. nil keeps the behavior (empty Peers, no
	// /metrics route); the Status/metrics guards set them so Status() returns a real
...
// feeds the /livecheck Peers list and metrics is the optional /metrics
// handler. nil for either keeps the default behavior.
func newTestNodeConfigured(t *testing.T, ca *crypto.MeshCA, dir, name string, peers []string, metrics http.Handler) *testNode {
	...
	// handler (nil disables the route — the default).
...
// path cmd/sovereign-node's serveConn uses). Dispatch: a BatchEnvelope
...
// TestClientInsertGet spins up an in-process node, dials it via the SDK,
...
func TestClientInsertGet(t *testing.T) {
	...
	t.Logf("PASS: 100 key/val pairs round-tripped on the originator; MerkleRoot stable (%s)", r1)
}

// TestClientRejectsUnsigned dials with NO client cert and asserts the
// TLS handshake FAILS — the mTLS guard (RequireAndVerifyClientCert) the server
...
func TestClientRejectsUnsigned(t *testing.T) {
	...
	// first request (the mTLS guard fires at the TLS layer).
	...
	t.Logf("PASS: no-client-cert dial rejected by the mTLS gate (RequireAndVerifyClientCert)")
}

// TestClientGetOnPeerReturnsDigestNotValue is the LOAD-BEARING honesty
// guard. It runs a SECOND node, gossip-converges it, then Gets a key ON THE
// PEER and asserts Payload="" AND PayloadDigest!="" — the value was discarded
// per the read-path design; the digest survives. A Get that returns a value string on a
...
func TestClientGetOnPeerReturnsDigestNotValue(t *testing.T) {
	...
	// Gossip-converge A<->B so B receives the delta (digest only by design).
	if !converge(t, a, b, 20) {
		ra := a.engine.State().MerkleRoot()
		rb := b.engine.State().MerkleRoot()
		t.Fatalf("did NOT converge in 20 rounds (rootA=%x rootB=%x) — the peer Get guard needs convergence", ra, rb)
	}
	...
	// THE GUARD: on a peer the payload is "" (the peer-side discard rule) and the digest
	// is non-empty. A non-empty Payload here is the fabrication the gate catches.
	if got.Payload != "" {
		t.Fatalf("FABRICATION: Get on peer returned Payload=%q — the value was discarded per the design rule; only the digest survives on a peer", got.Payload)
	}
	if got.PayloadDigest == "" {
		t.Fatal("Get on peer returned empty PayloadDigest — the digest MUST survive on a peer (the design rule)")
	}
	t.Logf("PASS: peer Get returns Payload=\"\" AND PayloadDigest=%s (the digest-only boundary is observable)", got.PayloadDigest)
...
// TestClientTLSThirteen0Only is a synthetic downgrade probe: it asserts
...
func TestClientTLSThirteen0Only(t *testing.T) {
	...
	t.Logf("PASS: negotiated TLS 1.3 (Version=%x); Min==Max==1.3 makes a downgrade impossible", st.Version)
}

// TestClientGetConcurrentSelfConsistent is the TOCTOU guard. It
...
// ONE PayloadForDot. This guard FAILS red on the double-State() code (the
...
func TestClientGetConcurrentSelfConsistent(t *testing.T) {
	...
	for i := 0; i < m; i++ {
		...
		if err != nil {
			...
			// tear; skip it. The self-consistency assert below is the guard.
			continue
		}
		...
		checked++
		// THE GUARD: the digest the response reports MUST be the SHA-256 of the
	...
	t.Logf("PASS: %d originator Gets all self-consistent (sha256(payload)==PayloadDigest) under concurrent inserts — no TOCTOU tear", checked)
}

// TestClientMetricsLabelPreserving is the label-collapse guard. It
...
// — silent data loss in the SDK's own metrics reader. The label-preserving parser change
// keeps every series; this guard FAILS red on the old map[string]float64 parser
...
func TestClientMetricsLabelPreserving(t *testing.T) {
	...
	if len(verdictSamples) != len(verdicts) {
		t.Fatalf("FAIL: Metrics() returned %d samples for sovereign_ingest_verdicts_total, want %d (a label-collapse parser keeps only the last-wins scalar — the six verdict labels did NOT survive)", len(verdictSamples), len(verdicts))
	}
	...
	t.Logf("PASS: all %d verdict labels survive Metrics() with their driven counts (no label-collapse data loss)", len(verdictSamples))
}

// TestClientMetricsMalformedNoPanic (self-check) drives Metrics() with
...
func TestClientMetricsMalformedNoPanic(t *testing.T) {
	...
	t.Logf("self-check PASS: Metrics() parsed %d samples from a malformed scrape without panicking", len(samples))
}

// TestClientStatusSmoke is the Status() smoke test. It builds a node
// whose control server is wired with a REAL peer slice (the harness
...
func TestClientStatusSmoke(t *testing.T) {
	...
	if len(st.Peers) == 0 {
		t.Fatal("Status returned an empty Peers slice — the control server was wired with nil peers (the design flaw); a real peer slice must survive")
	}
	...
	t.Logf("PASS: Status() returns NodeID=%s, Peers=%v, TLS1.3 (non-empty Peers — Status() is covered)", st.NodeID, st.Peers)
}

// TestDialWithCertsLoadsFiles is the DialWithCerts smoke test. It
...
func TestDialWithCertsLoadsFiles(t *testing.T) {
	...
	t.Logf("PASS: DialWithCerts loaded on-disk cert/key/ca, dialed, and round-tripped a Get (payload=%q)", got.Payload)
}
