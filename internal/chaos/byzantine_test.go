...
// Byzantine-attack acceptance + regression tests. See internal/chaos/byzantine.go
// for the admission-layer note (the physical-clock cap does not catch the
// logical-counter ratchet; the per-peer EWMA budget PeerBucket.Accept does).
...
// the A2 DotCounter ratchet is caught by the per-peer EWMA budget
// (PeerBucket.Accept) returning Drop in 1 admit.
//
// The physical-clock cap is independent of DotCounter (it rejects on
// physical-clock drift, not logical-counter ratchet); the catch is the
// per-peer EWMA budget. This test asserts PeerBucket.Accept returns Drop; it
// does NOT assert the physical-clock cap drops.
func TestByzantine_A2RatchetCaughtByAdmission(t *testing.T) {
	...
	// DotCounter to the per-peer EWMA budget PeerBucket.Accept. A MaxUint64 ratchet on a fresh
...
// the re-signed frame is CRYPTOGRAPHICALLY VALID even though
...
// TestByzantine_NoInPlaceDeltaMutation is the no-in-place-mutation guard: the injector
...
// TestByzantine_NoVectorClockLamportTime parses internal/chaos/byzantine.go
// (non-test) via go/ast and FAILS if `VectorClock` or `LamportTime` appears in
// CODE (allowed in comments — the comment-span exclusion). These identifiers
// are banned here; the injector MUST use only
...
// TestByzantine_NoVectorClockLamportTime_NegativeControl proves the guard is
...
func TestByzantine_NoVectorClockLamportTime_NegativeControl(t *testing.T) {
	...
	if len(violations) == 0 {
		t.Fatal("negative control FAILED: injecting `var VectorClock int` did NOT trip the guard — the guard is not load-bearing")
	}
	...
	if len(violationsReverted) != 0 {
		t.Fatalf("reverted source still trips the guard: %v — the guard is too broad (flagging comments)", violationsReverted)
	}
}

// TestByzantine_UsesRandV2 parses byzantine.go and FAILS if `rand.Seed(`,
// `mrand.Seed(`, or a bare `"math/rand"` import (not `"math/rand/v2"`) appears.
// math/rand/v2 is REQUIRED for any randomness in byzantine.go.
func TestByzantine_UsesRandV2(t *testing.T) {
	...
	for _, imp := range f.Imports {
		...
		if path == "math/rand" {
			t.Fatalf("banned bare \"math/rand\" import in byzantine.go — use \"math/rand/v2\" (the Go-1.20 deprecation ban, )")
		}
	}
	...
	if len(violations) > 0 {
		t.Fatalf("banned deprecated Seed calls in byzantine.go CODE: %v — use math/rand/v2 (no top-level Seed, )", violations)
	}
	...
	if !hasRandV2 {
		t.Fatal("byzantine.go does not import \"math/rand/v2\" — required for any randomness ()")
	}
}

// TestByzantine_UsesRandV2_NegativeControl proves the guard is
...
func TestByzantine_UsesRandV2_NegativeControl(t *testing.T) {
	...
	violations := scanBannedCallExprs(t, fset, f, []string{"rand.Seed", "mrand.Seed"})
	if len(violations) == 0 {
		t.Fatal("negative control FAILED: injecting `rand.Seed(1)` did NOT trip the guard — the guard is not load-bearing")
	}
}

// TestByzantine_CirclSignCited asserts byzantine.go contains a go-doc-style
// citation comment naming ed25519.Sign (circl v1.6.4) with the exact signature
...
func TestByzantine_CirclSignCited(t *testing.T) {
	...
	if !strings.Contains(s, wantSig) {
		t.Fatalf("byzantine.go missing the circl ed25519.Sign go-doc citation (exact signature %q) — the Sign symbol must be go-doc-proven, not fabricated ()", wantSig)
	}
	// Also require the file path citation so the symbol is traceable.
	if !strings.Contains(s, "ed25519.go:283") {
		t.Fatal("byzantine.go missing the ed25519.go:283 line citation for circl Sign ()")
	}
}

// TestByzantine_InjectorConcurrentNoRace is the race gate: 32
...
// sharded; the test exercises both under concurrency.
...
// decl before imports is a parse error). Used by the negative-control guards
// to inject banned code into CODE (not a comment) and prove the guard trips.
