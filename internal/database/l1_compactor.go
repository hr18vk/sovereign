...
// Package database (ADR-0019) implements the L0→L1 per-entity compaction.
//
// This eliminates the MaxL0Files silent-data-loss cap (ADR-0018 §6 /
...
// (superseded-row elimination) is a future Level-2 extension requiring truth-
...
// L1) but the L0 files are NOT deleted yet — delete-after-read-safety is left
// to a future reaper; the L0 files stay durable as the crash-recovery
// backstop.
package database
...
type CompactionConfig struct {
	...
	// extension if one file grows too large). The target is a SINGLE L1
	// file per entity — the full merged history. Default: 4.
	MaxL1FilesPerEntity int

	// EnableDominancePruning (ADR-0020) turns ON the Level-2
	// superseded-row prune at the compaction merge seam. false (the DEFAULT) →
	// Preserve-All: the BYTE-IDENTICAL baseline behavior (every row appended,
	// RowsPruned == 0, the same firstSysT, the same L1 bytes). The engine
	// NEVER auto-prunes — a horizon-less prune is SILENTLY WRONG (the // txTime-GAP proof). An operator must opt in AND set a positive
	...
	// PruningHorizonInt64Ns (ADR-0020) is the transaction-time GC
	...
	// queries. It is the load-bearing (C3) claw that closes the txTime
	...
	// future extension infers it off live-query txTime telemetry; here it is
	// taken as a config knob). 0 -> Preserve-All (with EnableDominancePruning=true) OR
	// Preserve-All (the default).
	PruningHorizonInt64Ns int64
	// PruneBackoffInt64Ns (ADR-0027) is the operator-set RETENTION
	...
	// "guess the frontier" (a static horizon) to "set the safety floor
...
//
// The default is Preserve-All (EnableDominancePruning=false) — byte-
// identical to the baseline behavior. There is NO behavioral change unless the
// operator sets a horizon (the engine NEVER auto-prunes). Per ADR-0027,
...
func DefaultCompactionConfig() CompactionConfig {
	return CompactionConfig{
		...
		// default (the back-compat guarantee).
...
// L1Compactor is the per-entity L0→L1 merger (ADR-0019).
//
...
//     depth: a per-entity file has only that entity's rows by
...
//
// Why preserve ALL rows (bitemporal truth-maintenance trap —): AsOf
...
// compaction PRESERVES ALL ROWS: merge = concatenate + sort by the
// composite key. Row-pruning (superseded-row elimination) is a future Level-2
// extension requiring truth-maintenance + a real DELETE operator — the dead
...
type L1Compactor struct {
	...
	// inferrer (ADR-0027) is the T_gc auto-inference state. The
	...
	// operator floor, so the txTime-gap silent-data-loss class the
	// prune guard closed STAYS closed (the inferrer never admits a horizon the operator's HARD
...
// horizonInferrer (ADR-0027) is the T_gc auto-inference state carried on
...
//
// ADR-0020: the LOUD horizon guard. An ENABLED prune
// (EnableDominancePruning=true) REQUIRES a positive PruningHorizonInt64Ns —
// the (C3) floor that closes the txTime gap. A horizon-less prune is
...
// (byte-identical baseline behavior), and the WARN states both the
...
func NewL1Compactor(lister S3Lister, downloader S3Downloader, uploader S3Uploader, allocator *JemallocAllocator, bucket string, cfg CompactionConfig) *L1Compactor {
	...
	// ADR-0027: capture the operator-set HARD floor + the backoff
	...
	// stays OFF (byte-identical baseline), and the inferrer's operatorFloor is
	...
	if cfg.EnableDominancePruning && cfg.PruningHorizonInt64Ns <= 0 {
		...
		// drop rows under a horizon-less (C1)&&(C2)-only rule — the // txTime-gap silent-data-loss class. Refuse the misconfiguration by
		...
		// importers + the mesh tests (mirrors the SetResolver seam):
		// the loudness is the log, not a new return type.
		log.Printf("[l1_compactor] WARN: EnableDominancePruning=true with PruningHorizonInt64Ns=%d (<=0) — a horizon-less prune is silently wrong (the txTime-gap class). Coercing to Preserve-All (EnableDominancePruning=false) — the L0→L1 merge preserves ALL rows (byte-identical behavior). Set --compaction-prune-horizon-ns to a positive transaction-time GC horizon to enable the Level-2 prune.",
...
// InferHorizon (ADR-0027) is the scheduler's single inferrer step: read
...
// pattern at query.go:253) — nil under the research node
...
// EffectiveHorizon (ADR-0027) computes the inferrer's effective
...
// SetInferredHorizon (ADR-0027) is the inferrer's monotonically-
...
// every error path must be loud; the silent-data-loss class the prune
// guard closed STAYS closed; the inferrer never silently lowers the horizon). A candidate
...
func (c *L1Compactor) SetInferredHorizon(candidate int64) int64 {
	...
	if candidate <= current {
		if candidate < current {
			...
			log.Printf("[l1_compactor] WARN: SetInferredHorizon retreat refused — candidate=%d < current=%d (a horizon retreat is a silent-data-loss contract violation; the monotone clamp refuses; investigate the query stream's txTime monotonicity)",
				candidate, current)
		}
...
type CompactionResult struct {
	...
	Rows         int      // rows preserved in the L1 (== RowsAfter; the back-compat field)
	AlreadyMoved bool     // true when the L0 set is empty or had zero entity rows (no-op)

	// ADR-0020: the Level-2 superseded-row prune counters. RowsBefore
	...
	// == 0 -- byte-identical baseline behavior (the back-compat guarantee).
	RowsBefore int
	RowsAfter  int
	RowsPruned int
}

// DominancePrune (ADR-0020) is the Level-2 superseded-row elimination
...
//     is LIVE -> drop corrupts (T1 RED — the txTime-GAP proof).
...
//
// WITHOUT (C3) the prune SILENTLY OVERWRITES the past (the trap):
...
// engine has long closed. (C3) is what refuses the drop when the
...
//
// COMPLEXITY (ADR-0025): the drop test for row R is an O(N*H) ACTIVE-
// DOMINATOR-SET SWEEP (H = average coverage depth — ADR-0020 §6(b) named the
// bound; the sweep realizes it). The sweep walks the production-precondition
...
// the differential-equivalence fuzz guard (>=10,000 cases) — a sweep
// that diverges on even ONE case is CORRUPT. The O(N^2) reference is preserved
// verbatim as the test-only dominancePruneReference check in
// l1_compaction_sweep_test.go.
//
// BACK-COMPAT: horizon <= 0 returns rows UNCHANGED (Preserve-All — the byte-
// identical baseline default; the caller gates on EnableDominancePruning too, so
...
func DominancePrune(rows []mergedRowT, horizon int64) []mergedRowT {
	if len(rows) == 0 || horizon <= 0 {
		...
		// slice unchanged (the caller appends every row — byte-identical baseline).
		return rows
	}
	// ADR-0025: the O(N*H) ACTIVE-DOMINATOR-SET SWEEP. Per ADR-0020
	...
	//
	// EQUIVALENCE (the transitivity proof, re-proved for the sweep).
	...
	// 0.002 — the BYTE-INVERTED prediction, corrected on audit). The
	// differential-equivalence fuzz guard (>=10,000 cases) GATES the byte-identical survivors.
	...
	// admit this row). This admission gate is the (C3) floor a guard pins.
	...
	// reference's in-place sink compaction (the differential-equivalence fuzz pins this).
	sink := 0
	for i := range rows {
		if !dominated[i] {
			if sink != i {
...
func (c *L1Compactor) Compaction(ctx context.Context, entityID string, entityHash8 [8]byte) (*CompactionResult, error) {
	...
	// ADR-0020: the Level-2 superseded-row prune pass — the SINGLE
	// minimum-blast-radius insertion point (between the
	...
	// the transaction-time GC FLOOR (the (C3) floor closes the txTime
	...
	// (the guards pin each claw). EnableDominancePruning=false (the DEFAULT) OR
	...
	// (delete-after-read-safety — a future reaper extension; they stay durable
	...
	return &CompactionResult{
		...
		Rows:        len(rows), // == RowsAfter (the back-compat field)
		RowsBefore:  rowsBefore,
		RowsAfter:   rowsAfter,
...
// buffer (ADR-0031 — the READ axis of the alloc cut). It replaces
...
// undo the win, as measured). Grows on demand for the rare larger manifest.
//
// MEASURED: at the production-relevant N=64 the E2E
...
// wire); this does NOT claim "zero allocs" — it claims the measured cut to the
// irreducible 1 (read) + 1 (string(body) in ParseManifest) + the l0Keys slice,
// with the Split-slice + the io.ReadAll doublings ELIMINATED. A future extension could
...
//
// ADR-0031: the implementation is a single-pass strings.IndexByte scan
...
// slice; the scan drops that slice — MEASURED −1 alloc/run at every N.
// The string(body) copy is KEPT (irreducible — the l0Keys
...
// there (the old substrings aliased the same string(body)). The alternative
// bytes.IndexByte+string(line)-per-L0 candidate was MEASURED 3-29× WORSE (a per-
// line []byte→string copy the old path never paid) — REFUTED by measurement, NOT shipped.
//
...
// is dropped, NOT fatal). Pinned by the byte-identity guard + the malformed-edge
// red control.
...
// PRESERVED: the per-entity construction guarantees only the one
