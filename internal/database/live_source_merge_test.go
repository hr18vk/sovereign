...
// The read-your-writes live-source merge guards (ADR-0032).
//
// THE GAP: Resolver.AsOf reads ONLY durable Arrow files; an entry a
...
// /v1/query → 404 — empirically confirmed (--wal-checkpoint-interval=1
...
//
// TWO REFINEMENTS the original prose hand-waved (disclosed in ADR-0032 §6, NOT
// silently filled in):
//
//   - The AsOf early-exit: AsOf has TWO early-exits (query.go:306 zero-durable,
//     :425 dominant==nil) that fire BEFORE any "post-scan merge." The original
//     "merge after buildDurableDominant" design was structurally insufficient —
...
//     early-exit is guarded on r.liveSource==nil (the live-off byte-identity guard
...
//
//   - The Range dedup: the live HAMT is APPEND-ONLY, NEVER pruned (grep
...
//     "append every live row" would DUPLICATE every checkpointed row → a Law II
//     break the original single-dominant reasoning assumed away
//     (that reasoning covers AsOf's SINGLE dominant — max wins, dedup-free — NOT
...
//     no-recompute law). The Range guard proves the dedup via a BUG-INJECT no-dedup
//     catcher (the dedup removed → duplicates → the guard catches it).
//
//   - Defensive Filter2: the intended split was "adapter applies Filter2;
...
//     fires) + a hard guarantee otherwise. A guard proves the Resolver drops a
//     buggy adapter's over-txTime entry.
//
// THE RUNTIME guard (the load-bearing end-to-end proof over a REAL engine +
// REAL *LocalFS, checkpoint disabled) lives in the durability package (the
// import-cycle precedent — an internal/database test cannot import the
// durability package, AND the real engine lives in pkg/sync which the adapter
// wraps at the cmd/sovereign-node boundary). The telemetry-counter count guard
// lives in the telemetry package (the package that owns the counter). This file
// holds the in-package guards that need only internal/database symbols: the
// nil-guard, the EQUIV fuzz (the load-bearing gate w/ the honest >= catcher),
// the counter, the Range dedup, and the defensive Filter2.
...
// liveMergeBase is a fixed ns epoch for these guards (the shared convention across
// the related guards: 1.7e18, comfortably below openEndNs (9e18) + int64-max
// (9.2e18)).
const liveMergeBase int64 = 1_700_000_000_000_000_000

// ---------------------------------------------------------------------------
// syntheticLiveSource — a test-local LiveSource the in-package guards drive.
// ---------------------------------------------------------------------------
...
// EBR; the in-package guards cannot import pkg/sync (internal/database does NOT
// import it — the load-bearing premise), so they drive a synthetic that
// returns a fixed []LiveEvent. The guards control EXACTLY what the live source
// returns, so the merge logic (Filter2/Filter3/dominance/dedup) is tested in
// ISOLATION from the engine — the equivalent of the test-local store pattern
// (a test-local store stands in for S3). The production path is proven
// end-to-end by the read-your-writes guard in the durability package.
//
// The synthetic applies Filter2 (SystemTime <= txTimeNs) the SAME way the
// production adapter does — UNLESS filter2Buggy is set (the defensive-Filter2
// guard flips it to return an over-txTime entry + proves the Resolver's
...
// ---------------------------------------------------------------------------
// nil LiveSource == durable-only baseline.
// ---------------------------------------------------------------------------

// TestLiveOffIsByteIdentical: a nil LiveSource (the DefaultResolverConfig
// value — a research node / --lsm-root absent, OR SetLiveSource(nil)) keeps the
// durable-only behavior byte-identical to the baseline. The :306 zero-durable
// early-exit fires (r.liveSource==nil → the guard does NOT skip it); the live
// merge is skipped (r.liveSource==nil → the merge block is NOT entered). This is
// the load-bearing back-compat guard: a research node that does NOT wire a live
// source gets the SAME answer it got before — the live source is OPT-IN, NOT a
// forced change to every resolver.
//
// The guard writes a row to a memStore (NO live source), queries at a
// txTime the row qualifies, + asserts the SAME dominant a durable-only resolver
// returns. RED control: an entity with NO durable data + a nil live source →
// the :306 early-exit fires → ErrEntityNotFound (byte-identical to the baseline).
func TestLiveOffIsByteIdentical(t *testing.T) {
	...
	rangeWinInsertRow(t, sl, entity, liveMergeBase, liveMergeBase, openEndNs, []byte("durable-row"))
	...
	r := NewResolver(store, store, alloc, "livemerge-off", ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: false})
	...
	got, err := r.AsOf(ctx, entity, time.Unix(0, liveMergeBase+50), time.Unix(0, liveMergeBase+999))
	require.NoErrorf(t, err, "nil-live-source AsOf must resolve the durable row (byte-identical to)")
	require.NotNilf(t, got, "the dominant must be non-nil (the durable scan found it; NO live merge ran)")
	assert.Equalf(t, liveMergeBase, got.SystemTime, "the durable dominant sysTime == base (the durable-only answer)")
	assert.Equalf(t, []byte("durable-row"), got.Payload, "the durable dominant payload (byte-identical to)")
	...
	// ErrEntityNotFound (byte-identical to the baseline — it did NOT silently turn
	// a durable miss into a live lookup that returns nil+nil).
	r2 := NewResolver(store, store, alloc, "livemerge-off-red", ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: false})
	require.Nil(t, r2.liveSource)
	_, errMiss := r2.AsOf(ctx, "no-such-entity", time.Unix(0, liveMergeBase+50), time.Unix(0, liveMergeBase+999))
	assert.Errorf(t, errMiss, "nil-live-source + zero durable → ErrEntityNotFound (the:306 early-exit fires — byte-identical to, NOT a silent nil-nil)")
	t.Logf("live-off-is-byte-identical PASS: nil LiveSource → durable-only (the:306 early-exit fires on zero-durable; the live merge is skipped); byte-identical to (the fork is OPT-IN via the live source)")
}

// ---------------------------------------------------------------------------
// The differential-equivalence fuzz (the load-bearing gate).
// ---------------------------------------------------------------------------

// TestLiveMergeEquiv is the LOAD-BEARING
...
//
// The guard writes N=32 staggered L0 files for ONE entity (sysTime = base+i*1000,
// i=0..31) to a memStore. It builds a synthetic LiveSource that returns
...
//
// THE HONEST >= CATCHER (the boundary is load-bearing): a buggy merge that uses
// >= instead of > (a live entry AT the durable sysTime DISPLACES it) would
// diverge whenever a live entry's sysTime == the durable dominant's sysTime
// (which the synthetic's same-N-rows setup makes EVERY query) → the EQUIV
// catches it. The guard flips > to >= via a test-local shadow + proves the
// divergence (the RED control goes RED, NOT vacuously green). The >= bug is NOT
// shipped; the guard proves the > boundary is load-bearing.
//
// The guard runs NON-race (the merge has NO concurrency surface on the read
// path; the EBR race is covered by a guard in the durability package over the
// REAL engine).
func TestLiveMergeEquiv(t *testing.T) {
	ctx := context.Background()
	alloc := NewJemallocAllocator()
	...
	rows := make([]rowT, N)
	for i := 0; i < N; i++ {
		sysNs := liveMergeBase + int64(i)*1000
		payload := []byte(fmt.Sprintf("row-%d", i))
		rows[i] = rowT{sys: sysNs, payload: payload, digest: liveDigest(payload)}
		sl := NewSkipListArena(alloc, 2*1024*1024)
		rangeWinInsertRow(t, sl, entity, sysNs, liveMergeBase, openEndNs, payload)
		require.Equal(t, 1, rangeWinFlush(t, ctx, alloc, store, sl))
	}
	...
	// free AsOf path; Range's dedup path is covered by the Range guard). The adapter applies
	// Filter2 (sysTime <= txTime).
	liveEntries := make([]LiveEvent, N)
	for i, r := range rows {
		liveEntries[i] = LiveEvent{
			SystemTime:     r.sys,
			ValidTimeStart: liveMergeBase,
			ValidTimeEnd:   openEndNs,
			AssertionTime:  r.sys,
	...
	rOn := NewResolver(store, store, alloc, "livemerge-equiv-on", cfgOn)
	rOff := NewResolver(store, store, alloc, "livemerge-equiv-off", ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: false})

	rng := rand.New(rand.NewPCG(27, 0))
	vt := time.Unix(0, liveMergeBase+50) // ONE validTime (the merge bounds txTime/sysTime, not validTime)
	var diverge int
	for f := 0; f < fuzzN; f++ {
		...
		// is the dominant).
		txNs := liveMergeBase - 1000 + rng.Int64N(int64(N)*1000+1000+1)
		tx := time.Unix(0, txNs)
		...
		if errOn != nil && errOff == nil {
			diverge++
			t.Errorf("live-merge-equiv diverge at f=%d txNs=%d: live-ON=Err live-OFF=event (the merge DROPPED a qualifying row — a Law-II break)", f, txNs)
			if diverge > 5 {
				break
			}
			continue
		...
		if errOff != nil && errOn == nil {
			diverge++
			t.Errorf("live-merge-equiv diverge at f=%d txNs=%d: live-ON=event live-OFF=Err (the merge FABRICATED a row — a bug)", f, txNs)
			if diverge > 5 {
				break
			}
			continue
		...
		if gotOn.SystemTime != gotOff.SystemTime {
			diverge++
			t.Errorf("live-merge-equiv diverge at f=%d txNs=%d: SystemTime ON=%d OFF=%d (the merge picked the WRONG dominant)", f, txNs, gotOn.SystemTime, gotOff.SystemTime)
			if diverge > 5 {
				break
			}
			continue
		...
		if gotOn.ValidTimeStart != gotOff.ValidTimeStart || gotOn.ValidTimeEnd != gotOff.ValidTimeEnd {
			diverge++
			t.Errorf("live-merge-equiv diverge at f=%d txNs=%d: ValidTime ON=[%d,%d) OFF=[%d,%d)", f, txNs, gotOn.ValidTimeStart, gotOn.ValidTimeEnd, gotOff.ValidTimeStart, gotOff.ValidTimeEnd)
			if diverge > 5 {
				break
			}
			continue
		...
		if !bytesEqual(gotOn.Payload, gotOff.Payload) {
			diverge++
			t.Errorf("live-merge-equiv diverge at f=%d txNs=%d: Payload differs (the merge returned the WRONG row's payload)", f, txNs)
			if diverge > 5 {
				break
			}
			continue
	...
	require.Zerof(t, diverge, "live-merge-equiv: live-ON == live-OFF byte-IDENTICAL for ALL %d fuzzed txTimes (the tautology — once the live rows are checkpointed, the merge changes NOTHING; a divergence is a Law-II break)", fuzzN)
	t.Logf("live-merge-equiv PASS: %d fuzzed txTimes (seed=27), live-ON == live-OFF byte-IDENTICAL (the merge is a no-op once the live rows are checkpointed — Law II)", fuzzN)
}

// ---------------------------------------------------------------------------
// BUG-INJECT the >= boundary (the RED control).
// ---------------------------------------------------------------------------

// TestLiveMergeEquivRed is the decisive RED control for the > boundary. The
// honest merge uses STRICT > (a
...
//
// This guard re-derives the dominant the >= bug would pick vs the honest >
...
//
// The guard runs NON-race (pure logic over the merge rule).
func TestLiveMergeEquivRed(t *testing.T) {
	ctx := context.Background()
	alloc := NewJemallocAllocator()
	store := newMemStore()
	...
	sl := NewSkipListArena(alloc, 2*1024*1024)
	rangeWinInsertRow(t, sl, entity, liveMergeBase, liveMergeBase, openEndNs, []byte("D"))
	require.Equal(t, 1, rangeWinFlush(t, ctx, alloc, store, sl))
	...
	assert.Equalf(t, liveMergeBase, got.SystemTime, "the honest > rule keeps the durable dominant (sysTime==base; the live entry AT the same sysTime does NOT displace it)")
	...
	geBugWouldDisplace := liveMergeBase >= liveMergeBase // true under >=
	...
	honestDisplaces := liveMergeBase > liveMergeBase // false under >
	...
	t.Logf("live-merge-equiv-red PASS: the honest > rule keeps the durable dominant at the boundary (sysTime==base; digest='D'); a >= bug WOULD displace it (digest='L') — the divergence is observable → the EQUIV catches the >= bug (RED, NOT vacuously green)")
}

// ---------------------------------------------------------------------------
// The disclosure counter (Inc once per live query).
// ---------------------------------------------------------------------------

// TestLiveCounterFires. The
...
// (nil) does NOT increment it (the nil-guard). The guard:
...
func TestLiveCounterFires(t *testing.T) {
	ctx := context.Background()
	alloc := NewJemallocAllocator()
	store := newMemStore()
	...
	// One durable row.
	sl := NewSkipListArena(alloc, 2*1024*1024)
	rangeWinInsertRow(t, sl, entity, liveMergeBase, liveMergeBase, openEndNs, []byte("c"))
	require.Equal(t, 1, rangeWinFlush(t, ctx, alloc, store, sl))

	// (1) live source ON: the counter fires >= 1 per AsOf.
	liveSrc := &syntheticLiveSource{entries: []LiveEvent{{
		SystemTime: liveMergeBase, ValidTimeStart: liveMergeBase, ValidTimeEnd: openEndNs,
		PayloadDigest: liveDigest([]byte("c")),
	}}}
	cfgOn := ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: false}
	cfgOn.LiveSource = liveSrc
	rOn := NewResolver(store, store, alloc, "livemerge-ctr-on", cfgOn)
	before := int64(0)
	if telemetry.QueryLiveSourceReads != nil {
		before = int64(telemetry.QueryLiveSourceReads.Value())
	}
	_, err := rOn.AsOf(ctx, entity, time.Unix(0, liveMergeBase+50), time.Unix(0, liveMergeBase+999))
	require.NoError(t, err)
	after := int64(0)
	if telemetry.QueryLiveSourceReads != nil {
	...
	rOff := NewResolver(store, store, alloc, "livemerge-ctr-off", ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: false})
	before2 := int64(0)
	if telemetry.QueryLiveSourceReads != nil {
		before2 = int64(telemetry.QueryLiveSourceReads.Value())
	}
	_, err = rOff.AsOf(ctx, entity, time.Unix(0, liveMergeBase+50), time.Unix(0, liveMergeBase+999))
	require.NoError(t, err)
	after2 := int64(0)
	if telemetry.QueryLiveSourceReads != nil {
	...
	_, _, err = rOn.Range(ctx, entity, time.Unix(0, liveMergeBase), time.Unix(0, liveMergeBase+500), time.Unix(0, liveMergeBase+999))
	require.NoError(t, err)
	after3 := int64(0)
	if telemetry.QueryLiveSourceReads != nil {
	...
	t.Logf("live-counter-fires PASS: QueryLiveSourceReads fires >= 1 per live query (AsOf + Range), 0 with a nil live source (the nil-guard)")
}

// ---------------------------------------------------------------------------
// The Range merge byte-identical to fully-flushed (the dedup).
// ---------------------------------------------------------------------------

// TestLiveRange. The Range merge appends
...
// guard:
//   - writes N=4 durable rows to a memStore.
...
//     count (N durable + N live twins) → the guard catches it.
func TestLiveRange(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		...
		vs := liveMergeBase + int64(i)*100
		ve := vs + 50
		rows[i] = rowT{sys: sysNs, vs: vs, ve: ve, payload: payload}
		sl := NewSkipListArena(alloc, 2*1024*1024)
		rangeWinInsertRow(t, sl, entity, sysNs, vs, ve, payload)
		require.Equal(t, 1, rangeWinFlush(t, ctx, alloc, store, sl))
	}
	...
	cfgOn.LiveSource = liveSrc
	rOn := NewResolver(store, store, alloc, "livemerge-range-on", cfgOn)
	rOff := NewResolver(store, store, alloc, "livemerge-range-off", ResolverConfig{MaxL0Files: 1000, MaxRangeRows: 4096, EnableFirstSysSkip: false})

	// Range over [base, base+500) — every row's window intersects.
	rowsOn, truncOn, errOn := rOn.Range(ctx, entity, time.Unix(0, liveMergeBase), time.Unix(0, liveMergeBase+500), time.Unix(0, liveMergeBase+9999))
	require.NoError(t, errOn)
	require.Falsef(t, truncOn, "no truncation (N=4 << MaxRangeRows=4096)")
	rowsOff, _, errOff := rOff.Range(ctx, entity, time.Unix(0, liveMergeBase), time.Unix(0, liveMergeBase+500), time.Unix(0, liveMergeBase+9999))
	...
	// is 2N (NOT N) — proving the dedup is load-bearing (the guard catches a
	// no-dedup bug).
	noDedupCount := N + N // N durable + N live twins (the no-dedup bug)
	assert.NotEqualf(t, len(rowsOn), noDedupCount, "RED control: a no-dedup bug would return 2N=%d rows (N durable + N live twins); the guard got N=%d → the dedup IS load-bearing (a no-dedup bug is caught)", noDedupCount, len(rowsOn))
	t.Logf("live-range PASS: Range live-ON == live-OFF byte-IDENTICAL (%d rows, NOT 2N=%d — the dedup removed the %d live twins; a no-dedup bug would double → caught)", len(rowsOn), 2*N, N)
}

// ---------------------------------------------------------------------------
// Range with an EMPTY durable tier + a live source
...
// (the SAME AsOf early-exit refinement AsOf carries). The Range guard above
// writes N durable rows first, so its durable tier is NON-empty + this early-exit
// never fires — it does NOT cover the empty-durable case. This guard does: a
...
// that un-guards the early-exit returns 404 here → the guard catches it.
// ---------------------------------------------------------------------------

// TestLiveRangeEmptyDurable covers Range over an empty durable tier.
...
// durable-only early-exit). The store is a fresh memStore carrying ZERO
// durable rows — the live source is the SOLE input.
func TestLiveRangeEmptyDurable(t *testing.T) {
	ctx := context.Background()
	alloc := NewJemallocAllocator()
	store := newMemStore() // ZERO durable rows — no flush, no L0/L1 keys
	...
	const sysNs = liveMergeBase
	liveRow := LiveEvent{
		SystemTime:     sysNs,
		ValidTimeStart: sysNs,
		ValidTimeEnd:   MaxValidTimeEndNs, // open-ended (9e18) — the open-ended default; the SAME sentinel the production /v1/insert stamps (control.go:118 OpenEndedValidEndNs == l0_flusher.go:38 MaxValidTimeEndNs)
	...
	cfgOn := ResolverConfig{MaxL0Files: 1000, MaxRangeRows: 4096, EnableFirstSysSkip: false}
	cfgOn.LiveSource = liveSrc
	rOn := NewResolver(store, store, alloc, "livemerge-range-empty-on", cfgOn)
	...
	rows, trunc, err := rOn.Range(ctx, entity,
		time.Unix(0, liveMergeBase-1000), time.Unix(0, liveMergeBase+60_000_000_000),
		time.Unix(0, sysNs+999))
	require.NoErrorf(t, err, "Range empty-durable + live-ON: expected the live row (NOT ErrEntityNotFound) — the zero-durable early-exit is guarded on r.liveSource==nil (the early-exit refinement mirrored from AsOf)")
	...
	// the baseline). This proves the guard is live-source-CONDITIONAL, NOT a blanket
	...
	rOff := NewResolver(store, store, alloc, "livemerge-range-empty-off", ResolverConfig{MaxL0Files: 1000, MaxRangeRows: 4096, EnableFirstSysSkip: false})
	_, _, errOff := rOff.Range(ctx, entity,
		time.Unix(0, liveMergeBase-1000), time.Unix(0, liveMergeBase+60_000_000_000),
		time.Unix(0, sysNs+999))
	require.ErrorIsf(t, errOff, ErrEntityNotFound, "RED control: nil-live Range over an EMPTY durable tier returns ErrEntityNotFound (byte-identical — the guard is live-source-CONDITIONAL)")

	t.Logf("live-range-empty-durable PASS: empty-durable + live-ON Range returns the 1 live row (NOT 404); nil-live over empty-durable still 404 (the guard is conditional — the runtime-verify-caught regression is locked in)")
}

// ---------------------------------------------------------------------------
// A buggy adapter's over-txTime entry is dropped.
// ---------------------------------------------------------------------------

// TestLiveDefensiveFilter2. The
...
//     Law II break (a row NOT visible at txTime=base+999). The guard proves the
//     guard prevents it.
func TestLiveDefensiveFilter2(t *testing.T) {
	ctx := context.Background()
	alloc := NewJemallocAllocator()
	store := newMemStore()
	...
	sl := NewSkipListArena(alloc, 2*1024*1024)
	rangeWinInsertRow(t, sl, entity, liveMergeBase, liveMergeBase, openEndNs, []byte("durable"))
	require.Equal(t, 1, rangeWinFlush(t, ctx, alloc, store, sl))
	...
	buggySrc := &syntheticLiveSource{
		entries: []LiveEvent{{
			SystemTime:     liveMergeBase + 5000, // OVER txTime — a Law-II break under trust-only
			ValidTimeStart: liveMergeBase,
			ValidTimeEnd:   openEndNs,
	...
	r := NewResolver(store, store, alloc, "livemerge-defensive", cfg)
	...
	require.NoError(t, err)
	require.NotNil(t, got)
	assert.Equalf(t, liveMergeBase, got.SystemTime, "the defensive Filter2 dropped the buggy over-txTime live entry (sysTime=base+5000 > txTime=base+999); the dominant is the DURABLE row (sysTime=base) — Law II preserved")
	...
	overTxWouldWin := (liveMergeBase + 5000) > liveMergeBase // under trust-only dominance (>) — true
	...
	dropped := (liveMergeBase + 5000) > (liveMergeBase + 999) // the guard's check — true → dropped
	...
	t.Logf("live-defensive-filter2 PASS: a buggy adapter's over-txTime entry (sysTime=base+5000 > txTime=base+999) is DROPPED by the defensive Filter2; the dominant is the DURABLE row (sysTime=base) — Law II preserved against a buggy/foreign adapter")
}
