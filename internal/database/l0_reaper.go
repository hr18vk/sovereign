...
// Package database — the L0 reaper + the land-mine closure (ADR-0021).
//
// This closes the three residual items ADR-0020 §6 deferred:
//
...
//	    recovery backstop (delete-after-read-safety was deferred;
//	    ADR-0019 §6, ADR-0020 §6c). The l0/ directory grows monotonically
...
// L0Reaper is the cross-entity superseded-L0 disk-reclaim sweep
...
// (one FS root, one keyspace — mirrors the established wiring discipline).
...
// (default false — byte-identical to prior behavior, the backstop kept forever).
...
func (r *L0Reaper) Reap(ctx context.Context) ReapResult {
	...
	for _, manifestKey := range manifestKeys {
		...
		body, _ := readManifestBody(rc) // single-grow read, NOT io.ReadAll's doublings (ADR-0031)
		_ = rc.Close()
