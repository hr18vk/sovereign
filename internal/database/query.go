...
type ResolverConfig struct {
	// LiveSource (ADR-0032) is declared FIRST for fieldalignment (an
	...
	// This bounds memory and network usage before compaction is online.
	// Default: 1000. Set to 0 for unlimited (NOT recommended without ceiling index).
	MaxL0Files int

	// MaxRangeRows (ADR-0024) bounds the number of rows a single
	...
	// every survivor (the pre-skip behavior, the comparison path for the
	// differential-equivalence guard).
	//
	...
	// manifest-sidecar work (ADR-0029 §6.a), NOT this flag. The skip is
	...
	// range_test.go:74) does NOT affect its correctness.
	EnableFirstSysSkip bool

	// LiveSource (ADR-0032) is the read-your-writes seam: an interface
	...
	// the default, every IMMEDIATE /v1/query → 404 — empirically confirmed
	...
	// / --lsm-root absent) keeps the read-only-durable behavior byte-identical
	// (the 503-disabled honesty contract is at the control layer,
	// control.go:404; a configured-but-nil live source keeps the durable-only
	// answer honest, NOT a silent live miss).
	//
	// STRUCTURAL TWIN of EnableFirstSysSkip: a behavioral
	...
	// to the real precedent — SetResolver is a SETTER, not a ctor arg
...
// a pkg/sync.CRDTEntry copy — internal/database does NOT import pkg/sync
...
// discipline — control.go:441, Law V). Future work that surfaces live
// Payload through the query response is a SEPARATE seam; the live-source seam
// carries the digest (the no-recompute identity) + a nil Payload (the durable index's
...
type LiveEvent struct {
	...
	// fieldalignment-optimal layout). The DTO is allocated in a per-query slice (NOT a contended
...
// LiveSource is the read-your-writes seam (ADR-0032). The impl wraps the
...
// S3Deleter abstracts object deletion from object storage (ADR-0021). It
...
// Resolver implements the Bitemporal Query Resolver.
//
...
//
// IMPORTANT: This is the initial "sequential scan" implementation.
// Indexed access via the Ceiling data structure is deferred to future work.
//
// Zero-GC Compliance:
...
type Resolver struct {
	...
	// liveSource (ADR-0032) is the read-your-writes seam. nil = the
	// DefaultResolverConfig value (a research node / --lsm-root absent) → AsOf
	// + Range keep the durable-only behavior byte-identical. The
	...
	// pruningFrontierObserved (ADR-0027) is the observed live-query
	...
	// the §0.e read-path-zero-alloc discipline; measured by the alloc guard). The
	...
	// observed guard). fieldalignment: it sits on its own
...
// no heap (the §0.e read-path-zero-alloc discipline; the alloc guard measures 0
// delta vs the prior AsOf).
...
func NewResolver(lister S3Lister, downloader S3Downloader, allocator *JemallocAllocator, bucket string, config ResolverConfig) *Resolver {
	...
	return &Resolver{
		...
		bucket:     bucket,
		config:     config,
		liveSource: config.LiveSource, // (ADR-0032): the live δ-CRDT HAMT seam; nil = durable-only (DefaultResolverConfig)
	}
}

// SetLiveSource (ADR-0032) injects the live δ-CRDT HAMT seam AFTER
// construction. It is the SetResolver precedent carried to the live
...
//
// Sequential Scan Strategy (pre-compaction):
...
func (r *Resolver) AsOf(ctx context.Context, entityID string, validTime time.Time, transactionTime time.Time) (*TriTemporalEvent, error) {
	...
	// This matches the hash computation in MemTable.Write().
	...
	// (ADR-0027): observe this query's txTime into the live-query txTime
	...
	// This is the READ path, NOT the write-path zero-
	// alloc gate; the alloc guard measures a 0-alloc delta vs the prior AsOf.
	r.observeQueryTxTime(txTimeNs)

	// : List L0 + L1 files from S3.
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("query: cancelled before listing: %w", err)
	}

	// (ADR-0019): the read path now scans BOTH tiers.
	...
	//     NOT by MaxL0Files. Under the single-L1-per-entity shape there is
	//     ≤1 file per entity (the merged set); future tiered-L1 work (MaxL1FilesPerEntity)
	...
	//     The MaxL0Files cap now bounds the TAIL (the recent uncompacted
	...
	// List L0 UNBOUND (ADR-0019 §3.5). The cap (MaxL0Files) now bounds the
	...
	// Tail semantics mandate — and (b) mis-cap when most L0s are superseded by an
	...
	// (ADR-0032): this early-exit is GUARDED on the live source. With a
	...
	// (insert → IMMEDIATE query → 200, checkpoint disabled) depends
	...
	// byte-identical to the durable-only path (the live-off byte-identity guard).
	...
	// (ADR-0030): pass txTimeNs into loadSupersededL0Keys so the
	// manifest-channel download skip can bound the compaction manifests the
	// SAME way the file-skip bounds the L0/L1 scan keys (the SAME
	...
	// holds their rows (verbatim — preserve-all); scanning a superseded
	...
	// The MaxL0Files cap disclosure (ADR-0018 §6 → clarified in ADR-0019 §3.5):
	...
	// dropped from the scan — HONEST in a stronger sense: the L1 is
	...
	// silent-loss form previously disclosed; compaction is opt-in, the disclosure
	...
	// : Scan files and apply tri-temporal dominance relation.
	...
	for _, key := range scanKeys {
		...
		// (ADR-0029): transitively-safe download skip. The file's MIN
		...
		// ok=false → NO skip → the pre-skip download+scan path runs (a corrupt/
	...
	// (ADR-0032): the read-your-writes live merge. After the durable scan,
	...
	// writes the CRDTEntry verbatim, snapshot.go:447; the live-merge equivalence
	// guard). A
	...
	if r.liveSource != nil {
		...
		if lerr == nil {
			for _, le := range lcs {
				...
				// honest default; the merge-equivalence guard's BUG-INJECT flips > to
				// >= and proves the boundary is load-bearing).
...
// Range (ADR-0024) resolves EVERY durable row for entity E whose
...
// class this engine has closed). The carrier of bitemporal truth is
...
// in the window, so the durable SURFACE Range scans is byte-identical to
...
// the window or paginates; the pagination token is future work — §6). A guard
// asserts MaxRangeRows=4 over a 10-row history yields truncated=true + exactly 4.
//
...
func (r *Resolver) Range(ctx context.Context, entityID string, validTimeLo, validTimeHi time.Time, transactionTime time.Time) ([]*TriTemporalEvent, bool, error) {
	...
	// : list L1 + L0 — byte-identical to AsOf's listing discipline.
	if err := ctx.Err(); err != nil {
	...
	// The entity has NO durable data (neither L1 nor L0) on disk → honest miss.
	// (ADR-0032): this early-exit is GUARDED on the live source — the
	// SAME early-exit refinement AsOf carries (query.go:442). With a live
	...
	// to the durable-only path (the live-off byte-identity guard covers BOTH read paths).
	//
	...
	// not 3). The live-range guard used a NON-empty durable tier (or a
	...
	// (ADR-0030): pass txTimeNs (in scope at :512) so the manifest-channel
	// skip bounds compaction manifests the SAME way AsOf does (the manifest
	// skip bounds sysTime, NOT validTime — the SAME orthogonality the
	...
	// : scan files, collecting every window-intersecting row. The cap is
	...
	for _, key := range scanKeys {
		...
		// (ADR-0029): transitively-safe download skip — byte-identical to
	...
	// (ADR-0032): the read-your-writes live merge for Range. After the
	...
	//
	// THE DEDUP (load-bearing — the live ⊇ durable consequence the single-dominant
	// reasoning assumed away for Range): the live HAMT is APPEND-ONLY and NEVER
	...
	// Law II break the single-dominant reasoning glossed over (that reasoning
	// covers AsOf's SINGLE dominant — max wins, dedup-free — NOT Range's
	// MULTI-ROW slice). The dedup
	...
	// II; the Range merge guard). A live-source error NEVER fails the query (the durable
...
// CoalesceRange (ADR-0024 §1.1) is the OPTIONAL post-sort dominance
...
func CoalesceRange(rows []*TriTemporalEvent) []*TriTemporalEvent {
	...
	// projection (future work, disclosed in §6 — the default coalesce is the
...
// scanWindowFile (ADR-0024) opens a single Arrow IPC file and scans for
...
// scanWindowRecordBatch (ADR-0024) scans a single Arrow record batch and
...
//     Range window. The collision guard seeds exactly that collision and asserts no leak.
//
...
func (r *Resolver) scanWindowRecordBatch(
	...
	for row := 0; row < nRows; row++ {
		...
		// would otherwise leak the WRONG entity's rows. The collision guard seeds that.
		rowEntityID := entityIDCol.Value(row)
		if !bytes.Equal(rowEntityID, entityIDBytes) {
			continue
		}
...
// loadSupersededL0Keys (ADR-0019) lists the compaction manifests for
...
// work).
//
// (ADR-0030): the manifest-channel download skip. txTimeNs is the
...
// "{firstSys}.manifest") is the L1's MIN sysTime — the SAME field the
...
// EnableFirstSysSkip is true AND firstSys > txTimeNs (STRICT > — the
...
//	     share firstSysT) → the L1 is file-skipped by the scan-loop skip.
...
//	     txTime → every listed L0 is file-skipped by the scan-loop skip.
...
//	     where the file-skip SKIPS them anyway (firstSys > txTime).
...
// for the SAME reason as the file-skip: a manifest whose firstSys == txTime points at an
...
// the SAME fallback the file-skip carries). Counted via
...
func (r *Resolver) loadSupersededL0Keys(ctx context.Context, l0Prefix string, txTimeNs int64) map[string]struct{} {
	...
	for _, mk := range keys {
		// (ADR-0030): the manifest-channel download skip — byte-identical
		// in SHAPE to the AsOf/Range file-skip block (the SAME
		...
		// DOWNLOAD count (the cost center this work closes), counted via
		...
		body, _ := readManifestBody(rc) // (ADR-0031): single-grow read, NOT io.ReadAll's doublings
		_ = rc.Close()
...
// durable Arrow key (ADR-0029). Grammar (byte-verified against the
...
//
// (ADR-0030): the parser is a PURE NUMERIC-TAIL reader (LastIndexByte('/')
...
// (byte-verified by the §0 probe + the manifest-parser guard). The
...
// The prior prose ("Manifests carry .manifest — NOT parsed by this
// helper") was a documentation error: the helper ALWAYS parsed ".manifest"
// (the grammar never excluded it); the manifest channel merely USES that property. The
...
// silently dropped on a parse failure (the file-skip and manifest-skip
// failsafe guards pin this).
//
...
// first row MIGHT qualify → DO NOT skip (the off-by-skip boundary guard).
//
...
// numeric-tail reader. The lexical-scan-order landmine (the reverse-lexical
...
// liveRowKey is the dedup key for the Range live merge (ADR-0032). The
