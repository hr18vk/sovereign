...
import (
	...
	"github.com/hr18vk/sovereign/internal/crypto"
...
type MemTable struct {
	...
	shutdownOnce    sync.Once     // Prevents double-close panic on shutdownChan
}
...
func (m *MemTable) Write(ctx context.Context, event TriTemporalEvent) error {
	...
	// runtime.KeepAlive(maskedStr) has been relocated to AFTER the Put()
	...
	// Bounded retry loop replaces single-retry to prevent stampede data loss.
	...
	for attempt := 0; err != nil && attempt < maxRetries; attempt++ {
		...
		if m.skipList == currentSL {
			if flushErr := m.swapAndFlushAsync(ctx); flushErr != nil {
				m.mu.Unlock()
				runtime.KeepAlive(maskedStr) // ensure maskedStr outlives all Put attempts
				return flushErr
			}
		}
		m.mu.Unlock()
	...
	// KeepAlive MUST be placed AFTER Put() completes.
...
func (m *MemTable) swapAndFlushAsync(ctx context.Context) error {
	...
	go func() {
		...
		// Per-entity streaming flush (ADR-0018):
...
func (m *MemTable) Flush(ctx context.Context) error {
	...
	// Per-entity streaming sync flush (ADR-0018): serialize each entity
...
func (m *MemTable) Close(ctx context.Context) error {
	m.closed.Store(true)
	// Protect channel close with sync.Once to prevent double-close panic.
