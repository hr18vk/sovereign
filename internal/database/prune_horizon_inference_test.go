...
// The T_gc auto-inference guards (ADR-0027).
//
// This closes the residual deferred from the Level-2/Level-3 prune work: the
// T_gc auto-inference (ADR-0020 §6a). The DominancePrune (C3) floor
// (CompactionConfig.PruningHorizonInt64Ns) was a STATIC operator knob; now the
// engine TRACKs the observed live-query txTime frontier (observed at
// AsOf-entry query.go:237) and feeds effective = max(operatorFloor,
...
// logged).
//
// The guards in THIS file use the package-private inferrer surface (the
...
// *LocalFS round-trip guard (§2.h) lives in
// pkg/durability (the import-cycle-forced home — the
// REAL-LocalFS helpers live there). The telemetry-counter +
// OTel-export guards (§2.f) live in internal/telemetry.
//
// PREMISE-AUDIT CORRECTION (ADR §7): §2.c
// (the equivalence guard) as originally framed feeds the SAME inferrer-computed horizon to BOTH
// the reference check AND the DominancePrune sweep. DominancePrune is a PURE
...
// to this guard (both sides get the same wrong int). The original claim that
// "§2.c might catch an inferrer off-by-one" is FALSE on the bytes. The
// off-by-one IS caught by the floor guard (asserts max(1000, 5000-500)==4500
// directly) + the REAL-LocalFS round-trip (at the inferred horizon).
// The equivalence guard's REAL value = proving the prune function is byte-identical
// UNDER THE NEW HORIZON SOURCE (the inferrer's max() output is a valid int64
// horizon the unchanged prune consumes, no type/order shift). This guard is
...
// carried by the guards that assert the inferrer's arithmetic DIRECTLY. Disclosed
// in ADR §7.
...
// ──────────────────────────────────────────────────────────────────────────
// §2.d  The query-path seam guard (§1.a).
//
...
// observed contract).
//
// This guard does NOT require a real *LocalFS: the observation seam
...
func TestInferAsOfObserves(t *testing.T) {
	...
	// The newLocalFS helper is in pkg/durability (unreachable here); we
	...
	if got := r.QueryTxTimeFrontier(); got != 0 {
		t.Fatalf("asof-observes: fresh Resolver frontier=%d, want 0 (no AsOf served yet)", got)
	}
	...
	for _, txNs := range increasing {
		...
		if got := r.QueryTxTimeFrontier(); got != txNs {
			t.Fatalf("asof-observes: after AsOf(txTime=%d) frontier=%d, want %d (the observation is a MAX; each increasing txTime advances it)", txNs, got, txNs)
		}
	}
	// The frontier equals the LAST (highest) txTime.
	if got := r.QueryTxTimeFrontier(); got != increasing[len(increasing)-1] {
		t.Fatalf("asof-observes: frontier=%d after the increasing sequence, want %d (the LAST/highest)", got, increasing[len(increasing)-1])
	}
	...
	if got := r.QueryTxTimeFrontier(); got != highWater {
		t.Fatalf("asof-observes: after a FORENSIC AsOf(txTime=%d) frontier=%d, want %d UNCHANGED (the observation is a MAX, not a last-writer; a past query must NOT collapse the frontier — the §0.b monotone contract)", forensic, got, highWater)
	}
	t.Logf("asof-observes PASS: increasing sequence advanced the frontier to %d; a forensic query at %d left it UNCHANGED (the observation is a MAX, not a last-writer)", highWater, forensic)
}

// TestInferAsOfObservesConcurrentRace drives the observation seam
...
func TestInferAsOfObservesConcurrentRace(t *testing.T) {
	r := NewResolver(noopLister22{}, noopLister22{}, NewJemallocAllocator(), "local", DefaultResolverConfig())
	ctx := context.Background()
	const entity = "alpha"
	vt := time.Unix(0, 500)
	...
	want := maxTx.Load()
	if got != want {
		t.Fatalf("asof-observes-concurrent-race: frontier=%d, want %d (the MAX txTime across %d goroutines × %d AsOf each; a torn atomic-max would lose the highest)", got, want, goroutines, perG)
	}
	t.Logf("asof-observes-concurrent-race PASS: %d concurrent AsOf goroutines, frontier=%d == the global MAX (0 races under -race; the CAS-loop atomic-max is race-free)", goroutines*perG, got)
}
...
// ──────────────────────────────────────────────────────────────────────────
// §2.e  The read-path-zero-alloc guard (§0.e).
//
...
// construction. The guard measures the AsOf alloc count in steady state (the
...
// disclosed ADR §6). The guard reports the EXACT number.
// ──────────────────────────────────────────────────────────────────────────

func TestInferAlloc(t *testing.T) {
	r := NewResolver(noopLister22{}, noopLister22{}, NewJemallocAllocator(), "local", DefaultResolverConfig())
	ctx := context.Background()
	const entity = "alpha"
	vt := time.Unix(0, 500)
	...
	// alloc cost (0 by construction; the guard PROVES it by measurement, not
	// assertion). The seam is the ONLY addition to the read path.
	allocsControl := testing.AllocsPerRun(runs, func() {
		_, _ = r.AsOf(ctx, entity, vt, tx)
	})

	t.Logf("infer-alloc: AsOf allocs/op WITH seam = %v (control = %v); the seam is a LoadInt64 + CompareAndSwapInt64 (the no-advance steady-state path) — ZERO alloc contribution by construction; measured", allocsWithSeam, allocsControl)

	// The seam's contribution is the delta. The two measurements are the SAME
	// path (the seam is idempotent); the delta is 0 by construction. The guard
	...
	if allocsWithSeam != allocsControl {
		t.Fatalf("infer-alloc: the observation seam added allocs: with-seam=%v control=%v delta=%v (the seam is a LoadInt64 + CompareAndSwapInt64 on a struct int64 — NO heap escape; a non-zero delta is a regression of the read-path zero-alloc discipline held since)",
			allocsWithSeam, allocsControl, allocsWithSeam-allocsControl)
	}
	t.Logf("infer-alloc PASS: the AsOf observation seam adds ZERO allocs/op (delta=0; with-seam=%v == control=%v) — the int64 lives on the Resolver struct, the CAS-loop is word-sized atomics, no escape (the §0.e read-path-zero-alloc discipline held)", allocsWithSeam, allocsControl)
}

// ──────────────────────────────────────────────────────────────────────────
// §2.a  The monotone guard (§0.c), the load-bearing RED control.
//
...
// a no-op, the retreat SUCCEEDS and this guard FAILS — the load-bearing proof.
// ──────────────────────────────────────────────────────────────────────────

func TestInferMonotone(t *testing.T) {
	...
	for i, cand := range sequence {
		...
		if got != expected[i] {
			t.Fatalf("infer-monotone step %d: SetInferredHorizon(%d) -> %d, want %d (the §0.c monotone clamp; a retreat to %d after %d MUST be refused + held at %d)",
		...
		if c.cfg.PruningHorizonInt64Ns != expected[i] {
			t.Fatalf("infer-monotone step %d: cfg.PruningHorizonInt64Ns=%d, want %d (the setter advances the field the prune reads; a retreat must NOT lower it)", i, c.cfg.PruningHorizonInt64Ns, expected[i])
		}
	}
	...
	if delta := postRefused - preRefused; delta != 2 {
		t.Fatalf("infer-monotone: PruningHorizonRetreatRefused advanced by %d, want 2 (the two refused retreats at 200 and 300; the §0.c LOUD-path counter — a retreat is refused + counted)", delta)
	}
	t.Logf("infer-monotone PASS: sequence %v -> effective %v (the two retreats at 200,300 refused); PruningHorizonRetreatRefused advanced by exactly 2 (the §0.c monotone clamp + LOUD refuse held)", sequence, expected)
}

// TestInferMonotoneRedControlNoClamp is the RED CONTROL for
// The monotone guard: a setter WITHOUT the monotone clamp (a no-op that accepts
// every candidate) would let the retreats SUCCEED. The guard drives the
// unclamped setter + asserts the cfg field RETREATS (the divergence the clamp
// prevents — the load-bearing proof). Modeled on the equivalence RED control.
func TestInferMonotoneRedControlNoClamp(t *testing.T) {
	...
	// retreats to 200 — the silent-data-loss class.
	setUnclamped(c, 200)
	...
	t.Logf("infer-monotone RED control: the no-clamp setter ACCEPTED the retreat 500->200 (cfg now %d) — the divergence the §0.c clamp prevents (a production setter refuses + counts)", c.cfg.PruningHorizonInt64Ns)
}

// ──────────────────────────────────────────────────────────────────────────
// §2.b  The floor guard (§0.b — the inferrer FLOORS the operator knob).
//
...
//
// PREMISE-AUDIT CORRECTION (ADR §7): the original §2.b says "A second CALL" —
...
// immutable). The guard builds TWO compactors (one per operatorFloor) and
// asserts EffectiveHorizon on each. The "second call" framing is a misread of
// the construction-time floor capture; the honest guard is two compactors.
// ──────────────────────────────────────────────────────────────────────────

func TestInferFloor(t *testing.T) {
	...
	if got1 != want1 {
		t.Fatalf("infer-floor case 1: EffectiveHorizon(5000) with floor=1000 backoff=500 = %d, want %d (max(1000, 5000-500); the inferred dominates, the operator floor is the backstop)", got1, want1)
	}
	...
	if got2 != want2 {
		t.Fatalf("infer-floor case 2: EffectiveHorizon(5000) with floor=6000 backoff=500 = %d, want %d (max(6000, 5000-500); the operator floor dominates — the inferrer NEVER goes below the operator hard floor)", got2, want2)
	}

	// Case 3 (the §6.c deferred case): operator floor BELOW the inferred frontier
	...
	if got3 != want3 {
		t.Fatalf("infer-floor case 3 (§6.c stale-floor): EffectiveHorizon(5000) with floor=100 backoff=500 = %d, want %d (the operator floor is below the inferred -> the inferred dominates; the operator floor is a no-op when stale)", got3, want3)
	}

	t.Logf("infer-floor PASS: case1 floor=1000 observed=5000 backoff=500 -> %d (inferred dominates); case2 floor=6000 -> %d (operator floor dominates); case3 floor=100 -> %d (stale floor is a no-op; §6.c) — the inferrer FLOORS the operator knob, never replaces it", want1, want2, want3)
}

// TestInferFloorRedControlIgnoreOperatorFloor is the RED CONTROL:
...
// ignored). The guard catches it — the §0.a operator-floor-ignored divergence.
func TestInferFloorRedControlIgnoreOperatorFloor(t *testing.T) {
	...
	// That is the silent-data-loss class: a horizon BELOW the
	...
	t.Logf("infer-floor RED control: the no-max inferrer produced %d (BELOW the operator floor %d) — the §0.a divergence the operator-floor backstop prevents (a production inferrer returns max(floor, inferred)=%d)", got, c.inferrer.operatorFloor, c.EffectiveHorizon(5000))
}

// ──────────────────────────────────────────────────────────────────────────
// §2.c  The equivalence re-gate (§1.c).
//
// PREMISE-AUDIT CORRECTION (ADR §7, the load-bearing one): §2.c as written
// feeds the SAME inferrer-computed horizon to BOTH the reference check AND the
...
// fail, and an inferrer off-by-one is INVISIBLE to this guard (both sides get
// the same wrong int). The original claim "§2.c might catch an inferrer off-by-
// one" is FALSE on the bytes.
//
// This guard is authored HONESTLY. Its REAL value: prove the prune function is
...
// carried by the floor guard (asserts the arithmetic DIRECTLY) + the REAL-
// LocalFS (the round-trip at the inferred horizon).
//
// seed=22 (ADR §6 names it); 10,000 cases.
// ──────────────────────────────────────────────────────────────────────────

func TestInferEquiv(t *testing.T) {
	const cases = 10_000
	r := newRandSeeded(22) // seed=22 (ADR §6 names it)
	...
	for c := 0; c < cases; c++ {
		...
		// discipline relies on). REUSED from the sweep fuzz.
		...
		if !survivorsByteIdentical(refOut, sweepOut) {
			t.Fatalf("infer-equiv: case %d DIVERGED at the inferrer-fed horizon. operatorFloor=%d backoff=%d observedFrontier=%d -> effective horizon=%d, N=%d.\nreference survivors: %s\nsweep survivors:     %s\nrows: %s",
	...
	t.Logf("infer-equiv PASS: %d fuzz cases, all byte-identical at the INFERRER-FED horizon (effective = max(operatorFloor, observedFrontier - backoff); the unchanged DominancePrune consumes the inferrer's max() output identically to the reference at the SAME horizon — the off-by-one burden is carried by the floor guard + the real-LocalFS round-trip guard, disclosed ADR §7)", cases)
}
...
// The guard's HONEST disclosure: this is the SAME formula the production
...
// ──────────────────────────────────────────────────────────────────────────
// §2.g  The retreat-counter loud-path guard (§0.c).
//
...
// operation = the counter stays 0 — a second sub-guard drives a monotone-
// advancing sequence and asserts the counter stays 0 (the GREEN control).
// ──────────────────────────────────────────────────────────────────────────

func TestInferRetreatCounterLoud(t *testing.T) {
	cfg := CompactionConfig{
		EnableDominancePruning: true,
	...
	if retreated != 5000 {
		t.Fatalf("retreat-counter-loud: the retreat SetInferredHorizon(100) returned %d, want 5000 (the §0.c clamp refuses + returns the current)", retreated)
	}
	...
	if delta := postRefused - preRefused; delta != 1 {
		t.Fatalf("retreat-counter-loud: PruningHorizonRetreatRefused advanced by %d, want 1 (the single refused retreat; the §0.c LOUD-path counter)", delta)
	}
	...
	if !strings.Contains(logText, "retreat refused") {
		t.Fatalf("retreat-counter-loud: the WARN log line was NOT emitted (got %q); the §0.c loud path logs the retreat refuse — every error path must be loud", logText)
	}
	t.Logf("retreat-counter-loud PASS: the retreat 5000->100 was refused (held at 5000); PruningHorizonRetreatRefused advanced by 1; the WARN log line emitted — the §0.c loud path (counter + log) held")
}

// TestInferRetreatCounterGreen is the GREEN control: a monotone-
...
func TestInferRetreatCounterGreen(t *testing.T) {
	...
	if delta := postRefused - preRefused; delta != 0 {
		t.Fatalf("retreat-counter-green: a monotone-advancing sequence advanced PruningHorizonRetreatRefused by %d, want 0 (zero retreats under correct operation — the §0.c GREEN control; a non-zero counter under a monotone txTime stream is a correctness smell)", delta)
	}
	t.Logf("retreat-counter-green PASS: a monotone-advancing sequence [100..5000] left PruningHorizonRetreatRefused UNCHANGED (delta=0) — the §0.c GREEN control (zero retreats under correct operation)")
}
