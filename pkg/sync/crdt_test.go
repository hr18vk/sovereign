...
// and BenchmarkCRDTEngine_Join). It is set to 2 GiB — the size Gate 2
// of proved holds the steady-state fill/reclaim equilibrium
...
const benchCRDTEngineArenaSize uintptr = 2 * 1024 * 1024 * 1024 // 2 GiB — see §3 Gate 2.
...
func TestCRDTEngine_ThreeNodeConvergence(t *testing.T) {
	...
	defer digestB.Release() // arena-backed digest release
	...
	defer digestC.Release() // arena-backed digest release
	...
	defer digestA.Release() // arena-backed digest release
	...
	defer digestB2.Release() // arena-backed digest release
	...
	for round := 0; round < 2; round++ {
		for i := 0; i < N; i++ {
			for j := 0; j < N; j++ {
				...
				delta.Release()
				digest.Release() // arena-backed digest release (loop: direct, not defer)
			}
		}
	}
...
func BenchmarkCRDTEngine_GenerateDelta(b *testing.B) {
	...
	// warm the delta/EBR steady-state pools (the deltaPool,
	...
	// 0-alloc gate steady-state, not cold-start. Mirrors the
	// warmEBRPool precedent (hamt_test.go:269). the INSERT setup above and
...
// (ADR-0034). The deleted primitive
