...
// — the Lamport skew bound (Byzantine A1 closure at the wire).
//
// This file is the production guards the mandate requires. It drives
...
// plus the concurrency guard (the design constraint) that proves the atomic snapshot's
// coherence.
//
// CASES:
//
...
// advance.
//
//	Case 3 — batched A1 at element 1, S1a atomic-reject (inheritance,
//
// the design constraint): a 3-element batch with element 1's DotCounter == bound+1
// (elements 0 and 2 consistent) returns an error naming element 1 and
// State().Get has ZERO new entries for all 3 entity IDs.
//
//	Case 4 — triply-corrupt frame ordering (O2 contract, the design constraint): a frame
//
// with digest mismatch + attribution mismatch + skew poison all on one
// frame reports the FIRST detected violation. Per's O1 contract,
...
//
// CASE 5 (concurrency guard, the design constraint — NOT a mutation):
//
...
// sentinels, rtDotOriginEntityID, rtHostA/rtHostB, rtSomeOtherNodeID, the build*
...
// alias-by-duplication is the-rejected anti-pattern). The guards
...
// is what the guards bite against.
package sync
...
// rtSkewEntityID is the single-event entityID for the guards —
// DISTINCT from rtEntityID and rtDotOriginEntityID so ApplyCRDTDeltaEvent writes
// into host B's state under its OWN key, isolating's join from any
...
const rtSkewEntityID = "tenant=acme;ledger=txn;id=lamport-skew-bound"

// rtSkewPoisonDotCounter is the far-future DotCounter the A1 attack
// stamps — a distinct, deliberately-implausible value chosen so the guards
// unambiguously catch a poisoned DataLoader frame. It is constructed in the
// test as receiver.bound + 1 (not a hardcoded constant) so the guards exercise
// the BOUND'S computation, not a magic-number comparison — the design constraint.
//
// rtSkewConsistentDotCounter is the happy-path DotCounter for Case 1 — a
...
// buildSkewWireFrame is the single-event wire-byte builder for the
// guards. It mirrors buildDotOriginWireFrame (so DotNodeID == OriginNodeID ==
...
func buildSkewWireFrame(t *testing.T, entityID string, originNodeID, dotNodeID [16]byte, digest [32]byte, payload string, dotCounter uint64) []byte {
	t.Helper()
...
// skewBatchSpec is the per-element override spec for buildSkewBatchFrame,
...
// skewBatchEntityID returns a distinct entityID per batch element derived
// from rtSkewEntityID so the far-side assertions isolate's batch.
func skewBatchEntityID(i int) string {
	return fmt.Sprintf("%s;el=%d", rtSkewEntityID, i)
}

// buildSkewBatchFrame is the batched wire-byte builder for Case 3.
...
func buildSkewBatchFrame(t *testing.T, specs []skewBatchSpec) []byte {
	t.Helper()
	msg, seg, err := capnp.NewMessage(capnp.SingleSegment(nil))
	if err != nil {
		t.Fatalf("capnp.NewMessage: %v", err)
...
// TestLamportSkewBoundRejects is the production guards: four
// cases + the concurrency guard, with three mutation proofs (A/B/C) named at
...
// failing line into the report. Restore before commit.
func TestLamportSkewBoundRejects(t *testing.T) {
	// ─────────────────────────────────────────────────────────────────────
	...
	case1Wire := buildSkewWireFrame(t, rtSkewEntityID, rtHostA, rtHostA, rtPayloadDigest, rtPayload, case1DotCounter)
	...
	gotC1 := engineB1.State().Get(rtSkewEntityID)
	if len(gotC1) != 1 {
		t.Fatalf("Case 1: State().Get(%q) has %d entries, want 1 — Join did NOT apply the validated clean frame; the bound closed A1 AND regressed the honest peer (a skew-bound violation: the bound must NOT reject a within-bound frame)", rtSkewEntityID, len(gotC1))
	}
	...
	t.Logf("Lamport-skew Case 1 GREEN: clean frame with within-bound DotCounter=%d applied; e.Lamport advanced 100 → %d (honest AdvanceLamportTo runs un-suppressed for non-poisoned frames — A1 closure does NOT regress the honest peer).", case1DotCounter, engineB1.LamportCounter())
	...
	// NOT advance" + "entry NOT in State" assertions fire. The guards bite a
	...
	// exactly the way/2f's bypass guards bit for digest/attribution.
	// ─────────────────────────────────────────────────────────────────────
	...
	case2Wire := buildSkewWireFrame(t, rtSkewEntityID, rtHostA, rtHostA, rtPayloadDigest, rtPayload, case2DotCounter)
	errC2 := engineB2.ApplyCRDTDeltaEvent(case2Wire)
	if errC2 == nil {
		t.Fatalf("LAMPORT-SKEW INVARIANT BROKEN: ApplyCRDTDeltaEvent accepted a far-future-stamped DotCounter=%d (bound=%d, bound+1) — MUTATION A/B CATCH: a neutered bound (`if false && …`) or a seam bypass (call ReconstructEntry directly) makes this fire: A1 is LIVE on the wire (the poisoned DotCounter reached Join and AdvanceLamportTo bricked the clock)", case2DotCounter, boundB2)
	}
	var wieC2 *WireIntegrityError
	if !errors.As(errC2, &wieC2) {
		t.Fatalf("LAMPORT-SKEW INVARIANT BROKEN: ApplyCRDTDeltaEvent rejected the far-future DotCounter with the wrong error type %T; expected *WireIntegrityError (the seam's typed error)", errC2)
	}
	if wieC2.Kind != WireIntegrityLamportSkewPoisoning {
		t.Fatalf("LAMPORT-SKEW Case 2 Kind: got %d, want %d (WireIntegrityLamportSkewPoisoning) — the far-future stamp must be classified on the skew axis, not as digest or attribution", wieC2.Kind, WireIntegrityLamportSkewPoisoning)
	}
	...
	if wieC2.ComputedBound != boundB2 {
		t.Errorf("Case 2 diagnostic ComputedBound: got %d, want %d — the error must carry the bound the wrapper computed (the skew-bound formula) so a reviewer can reconstruct", wieC2.ComputedBound, boundB2)
	}
	...
	gotC2 := engineB2.State().Get(rtSkewEntityID)
	if len(gotC2) != 0 {
		t.Fatalf("LAMPORT-SKEW INVARIANT BROKEN: after the far-future frame, State().Get(%q) has %d entries, want ZERO — MUTATION A/B CATCH: the poisoned frame reached Join (the bound was neutered or the seam was bypassed), A4 disk poisoning via AdvanceLamportTo is LIVE (the poisoned DotCounter would persist)", rtSkewEntityID, len(gotC2))
	}
	if postLamportC2 := engineB2.LamportCounter(); postLamportC2 != snapB2.Lamport {
		t.Fatalf("LAMPORT-SKEW INVARIANT BROKEN: e.LamportCounter advanced from %d to %d after a far-future frame — MUTATION A/B CATCH: the poisoned DotCounter reached Join→AdvanceLamportTo and bricked the receiver's clock (A1 LIVE, A4 persists the poisoned value to disk); the receiver can no longer mint a higher dot via NextDot", snapB2.Lamport, postLamportC2)
	}
	t.Logf("Lamport-skew Case 2 GREEN: ApplyCRDTDeltaEvent rejects a far-future DotCounter=%d (bound=%d) with %q; entry NOT applied, e.Lamport stayed at %d (A1 closed at the wire; A4 disk-poison prevented by construction).", case2DotCounter, boundB2, errC2.Error(), engineB2.LamportCounter())
	...
	// as for digest and for attribution).
	//
	// The SAME atomic snapshot is taken ONCE before the per-element loop
	// (the design constraint+8); the bound element 1 fails is the SAME bound elements 0
	...
	case3Specs := []skewBatchSpec{
		{entityID: skewBatchEntityID(0), originNodeID: rtHostA, dotNodeID: rtHostA, payload: rtPayload, payloadDigest: rtPayloadDigest, dotCounter: case3ConsistentDot},
		{entityID: skewBatchEntityID(1), originNodeID: rtHostA, dotNodeID: rtHostA, payload: rtPayload, payloadDigest: rtPayloadDigest, dotCounter: case3PoisonDot},
		{entityID: skewBatchEntityID(2), originNodeID: rtHostA, dotNodeID: rtHostA, payload: rtPayload, payloadDigest: rtPayloadDigest, dotCounter: case3ConsistentDot},
	}
	case3Wire := buildSkewBatchFrame(t, case3Specs)
	errC3 := engineB3.ApplyCRDTDeltaBatch(case3Wire)
	if errC3 == nil {
		t.Fatalf("LAMPORT-SKEW INVARIANT BROKEN: ApplyCRDTDeltaBatch accepted a batch whose element 1 is far-future-stamped (DotCounter=%d, bound=%d) — the per-element skew check was bypassed or neutered; S1a atomic-reject for the skew axis is LIVE-OPEN", case3PoisonDot, boundB3)
	}
	// The wrap must name element 1.
	if !containsSubstring(errC3.Error(), "element 1") {
		t.Fatalf("LAMPORT-SKEW Case 3 wrap: error %q does not name element 1 — the percent-w wrap must preserve the failing element index so the far-future stamp is attributable within the batch", errC3.Error())
	}
	var wieC3 *WireIntegrityError
	if !errors.As(errC3, &wieC3) {
		t.Fatalf("LAMPORT-SKEW Case 3: errors.As did not recover *WireIntegrityError from the wrapped batch error %T; the skew rejection must carry the seam's typed error for the skew axis", errC3)
	}
	if wieC3.Kind != WireIntegrityLamportSkewPoisoning {
		t.Fatalf("LAMPORT-SKEW Case 3 Kind: got %d, want %d (WireIntegrityLamportSkewPoisoning) — the batched far-future stamp must be classified on the skew axis", wieC3.Kind, WireIntegrityLamportSkewPoisoning)
	}
	...
	if postLamportC3 := engineB3.LamportCounter(); postLamportC3 != snapB3.Lamport {
		t.Fatalf("LAMPORT-SKEW INVARIANT BROKEN: e.LamportCounter advanced from %d to %d after a rejected batch containing a far-future element — the consistent element 0/2 was joined before the poisoned element 1 was detected (S1a broken)", snapB3.Lamport, postLamportC3)
	}
	t.Logf("Lamport-skew Case 3 GREEN: ApplyCRDTDeltaBatch rejects a 3-element batch whose element 1 is far-future-stamped (DotCounter=%d, bound=%d) with %q; ZERO new entries for all 3 entity IDs and e.Lamport stayed at %d (S1a atomic-reject survives on the skew axis, no partial apply).", case3PoisonDot, boundB3, errC3.Error(), engineB3.LamportCounter())

	// ─────────────────────────────────────────────────────────────────────
	// Case 4 — triply-corrupt frame ordering (O2 contract, the design constraint).
	//
	...
	// reported. Per's O1 contract the attribution check in
	...
	// FIRST. The skew wrapper runs AFTER ReconstructEntry (the design constraint / O2:
	...
	// first (Kind == 3) instead of WireIntegrityDotOriginMismatch (Kind == 2).
	// The O2 ordering guard bites an O2 re-ordering shipped as the public
	// path — a frame that already fails digest+attribution contributes
	// nothing to the skew concern, and the contract is a guard, not
	// stylistic.
	// ─────────────────────────────────────────────────────────────────────
	...
	case4WrongDigest := sha256Sum(t, []byte(rtPayload+"triply-corrupt-digest"))
	case4Wire := buildSkewWireFrame(t, rtSkewEntityID, rtHostA, rtSomeOtherNodeID, case4WrongDigest, rtPayload, case4DotCounter)
	errC4 := engineB4.ApplyCRDTDeltaEvent(case4Wire)
	if errC4 == nil {
		t.Fatalf("LAMPORT-SKEW INVARIANT BROKEN: ApplyCRDTDeltaEvent accepted a triply-corrupt frame (attribution + digest + skew all corrupt) — the seam must reject on the FIRST detected violation, not silently apply")
	}
	var wieC4 *WireIntegrityError
	if !errors.As(errC4, &wieC4) {
		t.Fatalf("LAMPORT-SKEW Case 4: errors.As did not recover *WireIntegrityError from %T", errC4)
	}
	// O2 contract: the FIRST reported kind is attribution (the O1 ordering fires
	...
	if wieC4.Kind != WireIntegrityDotOriginMismatch {
		t.Fatalf("LAMPORT-SKEW INVARIANT BROKEN: triply-corrupt frame reported Kind=%d (%s), want %d (WireIntegrityDotOriginMismatch) — MUTATION C CATCH: if the skew wrapper re-orders the skew check BEFORE ReconstructEntry, this fires with Kind=WireIntegrityLamportSkewPoisoning (3); the O2 ordering contract (skew AFTER digest+attribution) is LIVE-OPEN", wieC4.Kind, kindName(wieC4.Kind), WireIntegrityDotOriginMismatch)
	}
	if wieC4.Kind == WireIntegrityLamportSkewPoisoning {
		t.Fatalf("LAMPORT-SKEW Case 4: triply-corrupt frame reported LamportSkewPoisoning — the O2 ordering was REGRESSED; skew must run AFTER digest + attribution, never first on a multiply-corrupt frame")
	}
	// Belt-and-braces: the entry must NOT be applied (attribution rejection is terminal).
	if gotC4 := engineB4.State().Get(rtSkewEntityID); len(gotC4) != 0 {
		t.Fatalf("LAMPORT-SKEW Case 4: State().Get has %d entries after a triply-corrupt frame — the rejection must be terminal (no Join)", len(gotC4))
	}
	t.Logf("Lamport-skew Case 4 GREEN: triply-corrupt frame reports WireIntegrityDotOriginMismatch FIRST (Kind=%d), NOT LamportSkewPoisoning — O2 ordering contract holds (skew AFTER digest+attribution; a frame clean on those two is the only one the skew check runs on).", wieC4.Kind)
}

// TestLamportSnapshotCoherence is the concurrency guard
// (the design constraint). is the FIRST integrity check depending on RECEIVER
...
//
// MUTATION (concurrency guard — NOT one of A/B/C; a separate regression): a
//
...
//	concurrency guard catches exactly that slide.
func TestLamportSnapshotCoherence(t *testing.T) {
	...
	case5Wire := buildSkewWireFrame(t, rtSkewEntityID, rtHostA, rtHostA, rtPayloadDigest, rtPayload, poisonDot)
	...
	go func() {
		...
		for !stop.Load() {
			_ = engine.InsertLocal("lamport-skew-concurrency-storm", entry)
		}
	}()
	...
	// lamport than the pinned snapshot — this is what makes the guard NON-
	...
	if lamportAtApply <= snap.Lamport {
		t.Fatalf("LAMPORT-SKEW CONCURRENCY GUARD (precondition): the InsertLocal storm did NOT advance e.Lamport above the pinned snapshot=%d (observed=%d) — the guard would be vacuous; rerun (the storm goroutine was starved). This is a test-harness scheduling flake, not a lamport-skew defect; document and retry.", snap.Lamport, lamportAtApply)
	}
	...
	if err == nil {
		t.Fatalf("LAMPORT-SKEW CONCURRENCY GUARD BROKEN: ReconstructEntryWithSkewBound accepted a far-future DotCounter=%d (bound=%d) under concurrent InsertLocal — the bound was neutered OR a torn read made the bound incoherently loose", poisonDot, bound)
	}
	if rec != (ReconstructedEntry{}) {
		t.Fatalf("LAMPORT-SKEW CONCURRENCY GUARD BROKEN: ReconstructEntryWithSkewBound returned a non-zero rec on rejection — the wrapper must return (ReconstructedEntry{}, *WireIntegrityError) on a skew-poisoned frame (the value-return change value-return: the != nil check translates to a non-zero-value check, byte-identical semantics)")
	}
	var wie *WireIntegrityError
	if !errors.As(err, &wie) {
		t.Fatalf("LAMPORT-SKEW CONCURRENCY GUARD: errors.As did not recover *WireIntegrityError from %T", err)
	}
	if wie.Kind != WireIntegrityLamportSkewPoisoning {
		t.Fatalf("LAMPORT-SKEW CONCURRENCY GUARD: Kind got %d, want %d (WireIntegrityLamportSkewPoisoning) — the bound must reject the far-future frame even under concurrent InsertLocal", wie.Kind, WireIntegrityLamportSkewPoisoning)
	}
	...
	// instead of the pinned snapshot — failing here. The guard is NON-VACUOUS
	// exactly because the storm demonstrably moved the clock above the
	// snapshot.
	if wie.ReceiverLamport != snap.Lamport {
		t.Fatalf("LAMPORT-SKEW CONCURRENCY GUARD BROKEN: wie.ReceiverLamport=%d != pinned snapshot.Lamport=%d (storm drove e.Lamport to %d before the apply and %d after) — the wrapper lazily re-read e.lamport mid-call; under the concurrent InsertLocal storm the bound's ReceiverLamport field drifted from the caller's atomic snapshot. The snapshot is the load-bearing property of the first receiver-state-dependent integrity check; a lazy re-read makes it theater.", wie.ReceiverLamport, snap.Lamport, lamportAtApply, lamportAfterApply)
	}
	if wie.ReceiverLamport != 100 {
		t.Errorf("LAMPORT-SKEW CONCURRENCY GUARD (belt): wie.ReceiverLamport=%d, want exactly 100 (the pinned fresh-engine snapshot, not the storm-advanced value)", wie.ReceiverLamport)
	}
	t.Logf("Lamport-skew Concurrency Guard GREEN: under concurrent InsertLocal (e.Lamport advanced to %d before the apply and %d after), ReconstructEntryWithSkewBound reported wie.ReceiverLamport=%d == pinned snapshot.Lamport=%d (NOT a moved value); the snapshot is coherent, the wrapper is a pure function of (ev, snapshot), and the first receiver-state-dependent integrity check's load-bearing property holds.", lamportAtApply, lamportAfterApply, wie.ReceiverLamport, snap.Lamport)
}
