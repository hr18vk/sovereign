...
// — ARENA-EXHAUSTION RESOLUTION GUARD (non-production _test.go)
//
///scope: this file is the ONLY new source added by phase feat/-hamt-set-arena-steadystate.
// It contains NO production code. It does NOT modify crdt.go / hamt.go / hamt_arena.go /
// crdt_apply*.go / crdt_reconstruct*.go. It exposes exactly ONE audit guard that bites a
...
// compile its regex against the bench source and the guard goes RED before any
...
// N=1_000_000 ops — double the no-reclamation OOM threshold of ~500K
//testing.Benchmark honors only a 1s wall-clock
...
// (variable alloc)" at hamt_arena.go:329 — the same death site documented.
//
// the runtime drive is now guarded by raceEnabled (
...
// TestHAMTSetReclamationGuard pins the reclamation contract with
// a static source guard plus a forced-N runtime drive.
func TestHAMTSetReclamationGuard(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("HAMT-RECLAIM GUARD: cannot read hamt_test.go: %v", err)
	}
	benchStart := strings.Index(string(src), "func BenchmarkHAMT_Set(")
	if benchStart < 0 {
		t.Fatalf("HAMT-RECLAIM GUARD: BenchmarkHAMT_Set not found in hamt_test.go")
	}
	...
	if !retireRE.MatchString(benchBody) {
		missing = true
		t.Errorf("HAMT-RECLAIM GUARD: BenchmarkHAMT_Set is missing " +
	...
	if missing {
		...
		t.Fatalf("HAMT-RECLAIM GUARD: static guard FAILED — see errors above")
	}
	t.Logf("HAMT-RECLAIM GUARD (static): reclamation pair present in BenchmarkHAMT_Set")
	...
	//
	// under -race the framework-scaled testing.Benchmark call
	...
	// confused: a guard must AUDIT the bench, not INVOKE the bench
	...
	// but adds no race coverage. Per the precedent
	...
	// TestConcurrentJoinRace, and TestJoinParallelContentionCurve.
	if raceEnabled {
		t.Skip("HAMT-RECLAIM GUARD (forced-N drive): -race instrumentation perturbs " +
			...
			"TestConcurrentJoinRace / TestJoinParallelContentionCurve. " +
			"Mirrors the zero-alloc TestHotPathZeroAllocations precedent at " +
			"physics_test.go:198.")
	}
	...
	if err != nil {
		t.Fatalf("HAMT-RECLAIM GUARD: NewHamtArena: %v", err)
	}
	t.Cleanup(func() {
		if e := arena.Free(); e != nil {
			t.Errorf("HAMT-RECLAIM GUARD: arena.Free: %v", e)
		}
	})
	h := NewHAMT(arena)
	warmEBRPool(arena)
	...
	t.Logf("HAMT-RECLAIM GUARD (forced N=%d): arena reached steady state, no OOM", forcedN)
}
