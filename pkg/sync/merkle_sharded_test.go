...
// Guards for MerkleRootFromShards (pkg/sync/merkle_sharded.go).
// Same-package (package sync) so the tests reach the UNEXPORTED fields the
// production method + the no-arena-growth guard rely on:
...
// no-growth guard reads to PROVE MerkleRootFromShards does not move it
// (the external package mesh_test CANNOT reach these — it imports pkg/sync as
// engsync, the exported surface only).
//
// These guards are FALSIFIABLE + bug-inject-PROVEN (NOT tautologies):
// The byte-identity guard — the root equals State().MerkleRoot() byte-
...
// The no-arena-growth guard — the OOM gate: State() grows bumpOffset as
...
// guards NEED: InsertLocal mints only the LOCAL node's dot, so an all-InsertLocal
...
// the defect a prior draft of these guards shipped). foreignNodeIDs is the set
...
func TestShardedRootByteIdentity(t *testing.T) {
	if testing.Short() {
		t.Skip(" byte-identity guard inserts 100 keys across shards; skip in -short")
	}
...
func TestShardedRootByteIdentityFuzz(t *testing.T) {
	if testing.Short() {
		t.Skip(" fuzz byte-identity guard inserts 10K keys; skip in -short")
	}
	var localNode [16]byte
	localNode[0] = 0xee
...
// the RED control ever EQUALS State(), the guard is a TAUTOLOGY (the sort is not
...
func TestShardedRootByteIdentityRedControl(t *testing.T) {
	if testing.Short() {
		t.Skip(" RED control guard inserts 100 keys across shards; skip in -short")
	}
	var localNode [16]byte
	for i := range localNode {
	...
	if broken == want {
		t.Fatalf("RED CONTROL FAILED: the broken (shard-index) sort produced the SAME root as State().MerkleRoot()=%x\n"+
			"the guard is a TAUTOLOGY — the global (nodeID,counter) sort is NOT load-bearing. approach (i) is unproven.",
			want)
	}
	t.Logf("RED CONTROL PASS: the broken shard-index sort DIVERGES from State().MerkleRoot()=%x\n"+
		" broken (shard-index sort) = %x\n"+
		" → the global (nodeID,counter) sort IS load-bearing; the byte-identity guard is NOT a tautology.",
		want, broken)
}

// TestShardedRootNoArenaGrowth is the OOM gate (root cause A): State()
...
// guard PROVES it byte-level: bumpOffset after N MerkleRootFromShards() calls
...
//
// The arena is 256 MiB (the same the 8-node 10K loopback guard uses,
// join10KArenaSize) so the State() loop's merged views FIT for measurement
// — the GROWTH is asserted, NOT the OOM (the OOM is a separate guarded guard).
...
func TestShardedRootNoArenaGrowth(t *testing.T) {
	if testing.Short() {
		t.Skip(" no-arena-growth guard inserts 10K keys + runs a root-compute loop; skip in -short")
	}
	var nodeID [16]byte
	nodeID[0] = 0x42
	...
	if bumpAfterState <= bumpAfterSharded {
		t.Fatalf("RED SIGNAL MISSING (State path): bumpOffset did NOT grow across the State() loop\n"+
			...
			"expected State() (the defect) to ADVANCE bumpOffset — if it does not, the no-growth guard's\n"+
...
// TestShardedRootStateOOMObserved is the silicon-scale consequence guard: under
// a TIGHT arena (16 MiB) the SAME State() loop that the no-growth guard runs
// against a 256 MiB arena OOMs (hamt_arena.go:638 "arena exhausted (variable
// alloc)"). This is the GATE 1 root cause (A) asserting itself in-vitro.
// The panic is RECOVERED (defer recover) so the guard REPORTS the OOM as a PASS
...
// tight arena (the flat-path assertion in the no-growth guard already proves it
// byte-level; this guard adds the OOM-as-consequence observation).
func TestShardedRootStateOOMObserved(t *testing.T) {
	if testing.Short() {
		t.Skip(" State-OOM-observed guard runs a tight-arena State() loop; skip in -short")
	}
	var nodeID [16]byte
	nodeID[0] = 0x77
	...
	// arena-exhaust panic (the exact root-cause string). A non-OOM panic
	// (or NO panic) is a FAIL — the guard would otherwise be vacuous.
	oomObserved := false
	var oomMsg string
	func() {
		defer func() {
	...
	if !oomObserved {
		t.Fatalf("State-OOM-OBSERVED FAIL: the tight-arena (16 MiB) State() loop did NOT OOM with the\n"+
			...
			"expected the root cause (A) to assert itself in-vitro — if it does not, the OOM\n"+
			" is NOT reproduced at this scale + the guard's premise is stale (re-check the arena size).",
			oomMsg)
	}
	t.Logf("State-OOM-OBSERVED PASS: the tight-arena State() loop OOM'd with %q — the root cause (A)\n"+
...
// no-growth guard: a root-compute path that calls State() internally (the exact
// defect class the fix retires) MUST grow bumpOffset → the guard catches it. It
// proves the guard is NOT a tautology (a path that secretly builds the merged
// view is detected).
func TestShardedRootNoArenaGrowthRedControl(t *testing.T) {
	if testing.Short() {
		t.Skip(" no-growth RED control; skip in -short")
	}
	var nodeID [16]byte
	nodeID[0] = 0x99
	e := newShardedTestEngine(t, nodeID, 16*1024*1024)
	...
	// grows. The guard asserts the broken one is CAUGHT (grows), proving the
	// flat assertion in the main guard is load-bearing, not vacuous.
	brokenShardedThatCallsState := func() [32]byte {
		return e.State().MerkleRoot() // the defect: builds the merged view
	}

	bumpBefore := e.arena.bumpOffset.Load()
	// The defect wrapper OOMs a tight arena (the SAME root cause the main guard
	...
	if bumpAfter <= bumpBefore {
		t.Fatalf("RED CONTROL FAILED: the State()-calling 'sharded' wrapper did NOT grow bumpOffset\n"+
			...
			"the no-growth guard's flat assertion is VACUOUS — it would not catch a State()-calling wrapper.",
