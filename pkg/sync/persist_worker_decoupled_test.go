...
// — persistMu DISK-MUTEX DECOPLE (the LAST internal-engine sub-
// phase before). These guards pin the Path A architectural posture
...
// The guards are:
//
// - TestPersistWorkerDecoupledStatic: a STATIC regex guard
...
// - TestNoBlockingFsyncDrive: a runtime sanity drive that
...
// lenient on purpose — on NVMe this guard does NOT bite RED on pre-2.5c
// (the gate is satisfied incidentally); it is a sanity check that
// post-2.5c the t_MAX STAYS low.
// - TestDurabilityRoundTrip: the LOAD-BEARING runtime
...
//
// The guards do NOT downgrade red. They do NOT t.Skip under any condition
// other than testing.Short() (the static guard has NO race/skip guard; the
// runtime guards MAY skip the GOMAXPROCS-heavy parts under testing.Short()
// but the 1-NextDot durability sub-case MUST always run). NO fmt.Sprintf
// on any hot loop of these guards — stack buffers / strconv only (
// makeBinaryKey discipline; §1 scope Dijkstra amnesty applies — do NOT
// gate the guards on allocs/op, but DO NOT hot-allocate either).
...
// readCrdtSource is the single load point for the static guard's
...
// file via os.ReadFile (NOT go/embed — the guard must sweep the LIVE
// source on the current tree, not a stale build-time snapshot).
func readCrdtSource(t *testing.T) string {
	t.Helper()
	...
	if err != nil {
		...
		if err != nil {
			t.Fatalf("static static guard: cannot read crdt.go: %v", err)
		}
	}
	return string(data)
}

// TestPersistWorkerDecoupledStatic is the STATIC regex guard
...
// has NO race/skip guard — it pins the shape under every build mode.
func TestPersistWorkerDecoupledStatic(t *testing.T) {
	src := readCrdtSource(t)
	...
	nextDotBody := extractFuncBody(src, "NextDot", "CausalDot")
	advLamBody := extractFuncBody(src, "AdvanceLamportTo", "")
	...
	closeBody := extractFuncBody(src, "Close", "error")
	...
	for _, iv := range invs {
		if !iv.re.MatchString(src) {
			t.Errorf("PERSIST-DECOUPLE STATIC GUARD: %s FAILED — %s", iv.id, iv.got)
		}
	}

	// S5 / S6 scoped to the function body.
	if persistMuLock.MatchString(nextDotBody) {
		t.Errorf("PERSIST-DECOUPLE STATIC GUARD: S5 FAILED — NextDot body contains e.persistMu.Lock(); the decouple is undone (the CAS caller must NOT block on the disk mutex). NextDot body:\n%s", nextDotBody)
	}
	if persistMuLock.MatchString(advLamBody) {
		t.Errorf("PERSIST-DECOUPLE STATIC GUARD: S6 FAILED — AdvanceLamportTo body contains e.persistMu.Lock(); the decouple is undone. AdvanceLamportTo body:\n%s", advLamBody)
	}

	// S8 scoped to Close body.
	if !s8OnceClose.MatchString(closeBody) {
		t.Errorf("PERSIST-DECOUPLE STATIC GUARD: S8 FAILED — Close() body MISSING e.persistStopOnce.Do(func(){ close(e.persistCh) }) (idempotent worker stop). Close body:\n%s", closeBody)
	}
	waitIdx := s8Wait.FindStringIndex(closeBody)
	freeIdx := s8Free.FindStringIndex(closeBody)
	if waitIdx == nil {
		t.Errorf("PERSIST-DECOUPLE STATIC GUARD: S8 FAILED — Close() body MISSING e.persistWorkerWg.Wait(). Close body:\n%s", closeBody)
	}
	if freeIdx == nil {
		t.Errorf("PERSIST-DECOUPLE STATIC GUARD: S8 FAILED — Close() body MISSING e.arena.Free(). Close body:\n%s", closeBody)
	}
	if waitIdx != nil && freeIdx != nil && waitIdx[0] >= freeIdx[0] {
		t.Errorf("PERSIST-DECOUPLE STATIC GUARD: S8 FAILED — Close() body ordering: e.persistWorkerWg.Wait() MUST textually precede e.arena.Free() (drain BEFORE arena teardown). Close body:\n%s", closeBody)
	}

	// S9 scoped to both NextDot and AdvanceLamportTo bodies.
	if !s9Select.MatchString(nextDotBody) {
		t.Errorf("PERSIST-DECOUPLE STATIC GUARD: S9 FAILED — NextDot body MISSING non-blocking select { case e.persistCh <- nextLimit: default: }. NextDot body:\n%s", nextDotBody)
	}
	if !s9Select.MatchString(advLamBody) {
		t.Errorf("PERSIST-DECOUPLE STATIC GUARD: S9 FAILED — AdvanceLamportTo body MISSING non-blocking select { case e.persistCh <- nextLimit: default: }. AdvanceLamportTo body:\n%s", advLamBody)
	}
}

// extractFuncBody slices the source from the signature line of the named
...
func extractFuncBody(src, name, returnsSig string) string {
	_ = returnsSig
...
// TestNoBlockingFsyncDrive is the runtime sanity drive. It
...
// at NumCPU=32. The thresholds are LENIENT ON PURPOSE — on NVMe this guard
// does NOT bite RED on pre-2.5c (the gate is satisfied incidentally).
// Documented honestly in §6; it is NOT the load-bearing bite (is).
func TestNoBlockingFsyncDrive(t *testing.T) {
	if testing.Short() {
		t.Skip("PERSIST-DECOUPLE R3b no-blocking-fsync drive: GOMAXPROCS-heavy; skip in -short (the 1-NextDot durability sub-case in TestDurabilityRoundTrip always runs).")
	}
	numCPU := runtime.NumCPU()
	workers := numCPU
	t.Logf("PERSIST-DECOUPLE R3b: runtime.NumCPU()=%d; workers=%d", numCPU, workers)
	...
	if err != nil {
		t.Fatalf("PERSIST-DECOUPLE R3b: NewDeltaCRDTEngine: %v", err)
	}
	defer engine.Close()
	...
	t.Logf("PERSIST-DECOUPLE R3b: t_MAX (last NextDot per worker)=%s; threshold=%s (workers=%d, NumCPU=%d)", tMaxDur, threshold, workers, numCPU)
	if tMaxDur >= threshold {
		t.Errorf("PERSIST-DECOUPLE R3b: t_MAX=%s >= threshold %s — a CAS caller blocked on the persist path (the decouple may be undone; NextDot must NOT block on fsync)", tMaxDur, threshold)
	}
}

// TestDurabilityRoundTrip is the LOAD-BEARING runtime bite.
...
//
// The sub-case MUST always run — only the GOMAXPROCS-heavy parts of
// may skip under testing.Short(); this guard's 1-NextDot sub-case runs
// even in -short.
func TestDurabilityRoundTrip(t *testing.T) {
	numCPU := runtime.NumCPU()
	t.Logf("PERSIST-DECOUPLE R3c: runtime.NumCPU()=%d", numCPU)
	...
	t.Run("Steady_1001NextDot", func(t *testing.T) {
		...
		if recovered != want {
			t.Errorf("PERSIST-DECOUPLE R3c Steady: recovered Lamport counter=%d, want %d — Close() did not drain the persist worker before arena.Free (the durability round-trip broke)", recovered, want)
		} else {
			t.Logf("PERSIST-DECOUPLE R3c Steady: recovered Lamport counter=%d == want %d (Close-drain held)", recovered, want)
		}
	})
	...
	t.Run("OneNextDot_ThenClose_RaceSurface", func(t *testing.T) {
		...
		for iter := 0; iter < iterations; iter++ {
			...
			if recovered != want {
				...
				t.Logf("PERSIST-DECOUPLE R3c 1-NextDot iter %d: recovered=%d want=%d (Close-drain MISS — the worker was killed mid-fsync before arena.Free)", iter, recovered, want)
			} else {
				atomic.AddInt64(&sawOK, 1)
			}
		}
		t.Logf("PERSIST-DECOUPLE R3c 1-NextDot: iterations=%d ok=%d miss=%d (Close-drain must read 1001 every iteration; any miss is a decouple regression)", iterations, atomic.LoadInt64(&sawOK), atomic.LoadInt64(&sawMiss))
		if sawMiss != 0 {
			t.Errorf("PERSIST-DECOUPLE R3c 1-NextDot: %d/%d iterations missed the persisted value — Close() did not drain the persist worker before arena.Free (M4-style regression)", sawMiss, iterations)
		}
	})
}
