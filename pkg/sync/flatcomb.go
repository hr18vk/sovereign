...
// ---------------------------------------------------------------------------
//
// WHY THIS FILE EXISTS — architectural context (see scaling_gate_test.go
...
// scaling_gate_test.go commit notes). Go's sync/atomic operations are
...
// scaling gate's drainer) observe a monotonic generation even if a
// node index is recycled between two push CASes.
// - NullOffset64 (== 0) — empty-stack sentinel. Reused verbatim.
// - CacheLinePad — 64-byte pad.
//
// API PARITY with ElimStack (drop-in for the gate's gateStack
...
// both sides so concurrent head-walkers (diag_test.go, the scaling
...
// both the production code (via the gate's makeGateStack factory) and
