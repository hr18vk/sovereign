...
// ---------------------------------------------------------------------------
// RESIDENCY — THE CHAOS LAYER: Page-Fault Stress Gate (Blueprint Residency §1)
// ---------------------------------------------------------------------------
//
// Ruthless Go Engine Verification Blueprint, Residency, "The Threat of mmap and
...
//
// THE PHYSICS UNDER TEST (two distinct, both real, Residency threats)
//
// THREAT A — SCHEDULER STALL (the design doc's named concern):
...
// SIGSEGV-class failure Residency §2 will harden against at the process level,
...
JUDICIAL go tool trace CONFIRMATION (matches the design doc verbatim):
 ...
 go test -run TestResidencyPageFaultGate -trace=residency.trace./pkg/sync/
 3. Open:
 go tool trace residency.trace
 4. In "View trace", scope to the residency gate's eviction+traversal window
 and inspect "M (OS thread)" timelines. the design doc's mandate:
...
// residencyMaxStall is the design doc's absolute mandate: no OS thread may be
...
// Residency threats (control + data). 64 MiB → up to 16K 4 KiB pages.
...
// the design doc's concern is a blocked OS thread, and a blocked thread comes
...
// TestResidencyPageFaultGate is the design doc's Residency §1 gate. It:
//
...
func TestResidencyPageFaultGate(t *testing.T) {
	if testing.Short() {
		t.Skip("Residency gate: page-fault simulation; skip in -short")
	}
	...
	if wholeEvicted > 0 {
		...
		t.Logf("[3] host refused madvise(DONTNEED); cannot run the residency fault gate on this host")
		if os.Getenv("RESIDENCY_REQUIRE_MADVISE") == "1" {
			t.Fatalf("RESIDENCY_REQUIRE_MADVISE=1 but host refuses MADV_DONTNEED")
		}
	...
	if !status.Hardened() {
		...
		if os.Getenv("RESIDENCY_REQUIRE_MLOCK") == "1" {
			t.Fatalf("RESIDENCY_REQUIRE_MLOCK=1 set but host cannot pin (status=%+v): %s. %s",
	...
	// NOT pin the control region, which is the fatal defect the design doc
	// mandates must not occur.
	if pinned.faulted {
		t.Errorf("RESIDENCY GATE A (CORRECTNESS) FAILED: post-pin traversal PANICKED (control page zeroed). "+
	...
	if pinned.faultsOk {
		if pinned.majorFaults > 0 {
			t.Errorf("RESIDENCY GATE B (PHYSICS) FAILED: %d major page faults during the post-pin sweep — "+
		...
		t.Logf("✓ RESIDENCY GATE B (PHYSICS): zero major page faults over %d probes — the control path is pinned resident. "+
		...
		if pinned.p99 > fallbackCeil {
			t.Errorf("RESIDENCY GATE B (PHYSICS, fallback /proc) FAILED: post-pin P99 traversal = %v, exceeds the "+
		...
		t.Logf("✓ RESIDENCY GATE B (PHYSICS, /proc unreadable): post-pin P99 = %v <= 100µs fallback ceiling. Major-fault delta unavailable.",
			pinned.p99)
	}
	t.Logf("✓ RESIDENCY PASSED: mlock defeats BOTH the scheduler stall (Threat A) and the live-control corruption (Threat B) under the deterministic fault-delta gate. Judicial trace recipe:\n%s",
