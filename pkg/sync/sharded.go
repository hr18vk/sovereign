...
// ---------------------------------------------------------------------------
// — Sharded Elimination & Combining (SEC) Stack
// ---------------------------------------------------------------------------
//
// ARCHITECTURE
//
// This file implements the multi-locus sharded stack mandated by the
// Architecture Analysis. The single-locus pathology measured
// by scaling_diag_test.go is decisive:
//
...
// drainAll == netSurplus (§4). Per-shard LIFO satisfies both.
//
...
// BurstMax is the maximum burst size the crucible offers
...
//
// Sizing the cache to 2× BurstMax (1024) is the wrong reflex:
...
// anti-pattern the directive forbids.
//
// The principled size is the typical-burst scale (gateBurstMean*2,
...
// while the heavy tail (the rare K up to BurstMax) is allowed
// to spill through the BOUNDED refill path — the regime the auxiliary
// TestBurstAbsorptionGate names and measures separately as the
