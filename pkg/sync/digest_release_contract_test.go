...
// ---------------------------------------------------------------------------
// — Chaos-Digest Release Discipline: the static CONTRACT PIN.
// ---------------------------------------------------------------------------
//
// This is guard a STATIC regex guard that pins the Release-contract
...
// TestChaosDigestReleaseRoundTrip gate. Two guards, two axes: the 10K-round OOM drive bites on
// the leak rate at runtime; this guard bites on the source shape statically.
//
// The contract: for EVERY non-comment line that calls
// `.GenerateDigest()`, a `.Release()` call (deferred or direct) MUST appear
// WITHIN 3 LINES AFTER the GenerateDigest call. The guard loads the source as
...
//
// Walked files:
// - pkg/sync/crdt_test.go (the leak sites at 308/314/320/326/375)
// - internal/chaos/partition.go (the panic caller at 221)
//
// Mutation M2: comment out the FIRST digestB.Release() in crdt_test.go
// and re-run this guard — it MUST FAIL because the regex no longer matches
// every GenerateDigest-within-3-lines-of-Release site. Restore, GREEN.
//
// This guard does NOT downgrade red. It does NOT t.Skip under any condition
// other than testing.Short() (mirroring's raceEnabled discipline).
...
// guard still sees the StateDigest call on an uncommented line and bites
...
// line window is the contract the 2.5b.1 §mandate pins.
func auditFileForReleaseContract(t *testing.T, path string) {
	...
	for i := 0; i < len(lines); i++ {
		...
		// so comment-only lines are skipped — the guard bites the moment a
		...
		if !found {
			t.Errorf("callaudit: %s:%d calls GenerateDigest() but no.Release() within 3 lines after it:\n %d: %s\nThe bounded per-engine HamtArena makes a forgotten Release observable as `HamtArena: OOM`. Add `defer <digest>.Release()` (or a direct Release at the bottom of a loop iteration).",
				filepath.Base(path), i+1, i+1, line)
		}
	}
}

// TestGenerateDigestReleaseContractPin is the static contract pin.
...
func TestGenerateDigestReleaseContractPin(t *testing.T) {
	if testing.Short() {
		t.Skip("call-audit static callaudit scans source; skip in -short")
	}
	// pkg/sync/crdt_test.go — the leak sites (308/314/320/326/375).
	// Path is relative to the package dir (this file lives in pkg/sync).
	auditFileForReleaseContract(t, "crdt_test.go")
	// internal/chaos/partition.go — the panic caller (line 221).
