...
// Track 1.3 — the 120-byte CRDT frame factory for the ML-DSA byte-cost bench.
//
// This is the SAME 120-byte payload BOTH halves of the PQ gate sign:
//   - the ML-DSA-65 envelope (SignCRDTFrame_PostQuantum) signs this frame, and
//   - the comparator Ed25519 seam (VerifyCRDTFrame) signs the same 120B.
// Keeping the payload byte-identical makes the comparison a SIZE comparison (the
// load-bearing economic axis — a 3309B ML-DSA-65 sig vs a 64B Ed25519 sig on the
// same 120B payload), NOT a payload-shape comparison.
//
// LAYOUT (the ADR-10 CRDTEntry shape, 120 bytes, zero internal padding — the
// same shape pkg/durability120/frame_factory_test.go + pkg/codec120/
// frame_factory_test.go assemble; reconstructed here from REAL fixed-size
// fields, NOT imported from either, which are test-only and importing a test
// package across packages is a smell):
//
...
// zeros. The PQ signature is computed over the frame bytes; a zero-frame would
// not change the SIZE economics (the sig is 3309B regardless) but it would
// misrepresent the real payload. So the factory fills the digest + node IDs
// from a PRNG and the counters from monotone arithmetic — never make([]byte,
// 120) of zeros.
//
// DETERMINISM is load-bearing: a seeded splitmix64 makes every run byte-
// identical across runs given an equal seed, so the bench ns/op is STABLE (a
// randomized frame would vary per-run and the ns/op would not be reproducible).
...
// frame (and therefore the bench ns/op) is reproducible across runs.
...
// different values) — exactly the real payload regime. Two frames at the SAME
// seed are byte-identical (the determinism contract).
//
// Returns a [120]byte value (not a slice) so the caller can take its address
// for a zero-copy slice without an allocation, and so it matches the
// SignCRDTFrame_PostQuantum(sk, frame [120]byte, ...) signature exactly.
func MakeFrame120(seed uint64) [frameSize]byte {
	...
	// so only ~64 distinct origins, matching a real mesh.
...
// TestFrame120Determinism asserts the determinism contract BEFORE any ns/op is
// recorded: two frames at the same seed are byte-equal; two frames at differing
// seeds differ in >= 80 bytes (the high-entropy digest + node IDs alone span 64
// bytes, plus the counters/timestamps/H3 differ). This is the gate that the
// bench ns/op rests on a STABLE payload.
func TestFrame120Determinism(t *testing.T) {
	...
	if diffs < 80 {
		t.Fatalf("distinct-seed frames differ in only %d bytes (want >= 80); "+
			"the entropy shape is wrong (likely zeros) and the ns/op would not be "+
