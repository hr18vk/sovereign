...
// (ADR-0029) guards (the REAL *LocalFS route guards) — the filename-
// bounded download skip, driven against a REAL *LocalFS (the guard-
...
// imports internal/database). The seam-level guards (the parser, the failsafe,
// the EQUIV fuzz, the boundary, the frozen scope) live in
// internal/database/file_download_skip_test.go. This file holds the guards the import-
...
// / makePackedValue / putBE64) + the helpers (insertWindowRow
// / rangeWinBaseDur) — the SAME REAL *LocalFS + the SAME L0Flusher→Arrow→disk
// path the compaction/range guards drive. The skip is READ-only; it reuses the
// Resolver (NewResolver over the *LocalFS lister+downloader), NOT a new write
// path. Each insertWindowRow flushes ONE row → ONE L0 file keyed by that
...
//
// ADR-0029 §5 CONTRACT: the skip is a transitively-safe elimination. For a
...
// ---------------------------------------------------------------------------
// skip-preserves-answer (REAL *LocalFS) — the headline Law-II preservation.
// ---------------------------------------------------------------------------

// TestDownloadSkipPreservesAnswerRealLocalFS is the skip-preserves-
// answer guard over a REAL *LocalFS. Write N=4 staggered files for E at sysTime
...
// seam-guard (the internal/database skip-equiv guard) proved over the in-memory store,
...
func TestDownloadSkipPreservesAnswerRealLocalFS(t *testing.T) {
	ctx := context.Background()
	lfs := newLocalFS(t)
	alloc := database.NewJemallocAllocator()
	flusher := database.NewL0Flusher(alloc, lfs, "dl-bucket")
	const entity = "alpha"
	...
	assert.Equalf(t, rangeWinBaseDur+1000, withSkip.SystemTime,
		"the dominant sysTime == base+1000 (the highest sysTime <= txNs=base+1500; the base+2000/base+3000 rows are invisible at txNs)")
	t.Logf("skip-preserves-answer REAL *LocalFS: 4 staggered files, txTime=base+1500 → skip-path == no-skip-path byte-IDENTICAL (dominant sysTime=%d); the base+2000/base+3000 files skipped (Law II preserved)", withSkip.SystemTime)
}

// ---------------------------------------------------------------------------
// skip-download-count (REAL *LocalFS) — the disclosure counter + the cut.
// ---------------------------------------------------------------------------
...
// instrument the skip-download-count guard reads to ASSERT the skip CUT the
...
type countingLocalFS struct {
	*LocalFS
	downloads int // non-atomic: the guard is single-threaded (one AsOf at a time)
}
...
// TestDownloadSkipDownloadCountRealLocalFS is the skip-download-count
// guard over a REAL *LocalFS. Same N=4 staggered-files setup as the skip-preserves-
// answer guard. Query at txTime = base+1500 with EnableFirstSysSkip=true: the Download
...
// → the guard's downloads<=2 assertion catches it. A skip that cuts the download
// but does NOT fire the counter would show downloads=2 + skips=0 → the guard's
// skips>=2 assertion catches it. BOTH the cut AND the disclosure are asserted.
func TestDownloadSkipDownloadCountRealLocalFS(t *testing.T) {
	ctx := context.Background()
	lfs := newLocalFS(t)
	alloc := database.NewJemallocAllocator()
	flusher := database.NewL0Flusher(alloc, lfs, "dl-dc-bucket")
	const entity = "alpha"
	...
	skipR := database.NewResolver(skipLFS, skipLFS, alloc, "dl-dc-bucket", database.ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: true})
	skipsBefore := int64(0)
	if telemetry.QueryDownloadSkippedFirstSys != nil {
		skipsBefore = int64(telemetry.QueryDownloadSkippedFirstSys.Value())
	}
	got, err := skipR.AsOf(ctx, entity, time.Unix(0, rangeWinBaseDur+50), time.Unix(0, txNs))
	...
	noSkipR := database.NewResolver(noSkipLFS, noSkipLFS, alloc, "dl-dc-bucket", database.ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: false})
	skipsBefore2 := int64(0)
	if telemetry.QueryDownloadSkippedFirstSys != nil {
		skipsBefore2 = int64(telemetry.QueryDownloadSkippedFirstSys.Value())
	}
	got2, err2 := noSkipR.AsOf(ctx, entity, time.Unix(0, rangeWinBaseDur+50), time.Unix(0, txNs))
	...
	assert.Equalf(t, int64(0), skipsAfter2-skipsBefore2, "SKIP OFF: the disclosure counter fired 0 (no skip — the pre- behavior)")

	// byte-IDENTITY of the answer across the two paths (belt-and-suspenders with
	// skip-preserves-answer — the cut did NOT change the answer).
	assert.Equalf(t, got.SystemTime, got2.SystemTime, "the cut did NOT change the answer (downloads 4→2, dominant byte-identical)")
	t.Logf("skip-download-count REAL *LocalFS: SKIP ON → %d downloads + %d skips; SKIP OFF → %d downloads + %d skips (the cut 4→2 + the disclosure 0→2; Law II preserved: dominant sysTime=%d both paths)", skipLFS.downloads, skipsAfter-skipsBefore, noSkipLFS.downloads, skipsAfter2-skipsBefore2, got.SystemTime)
}

// ---------------------------------------------------------------------------
// skip-range-window-orthogonal (REAL *LocalFS) — the skip is window-agnostic.
// ---------------------------------------------------------------------------

// TestDownloadSkipRangeWindowOrthogonalRealLocalFS is the skip-range-
// window-orthogonal guard over a REAL *LocalFS. The skip bounds sysTime (Filter2);
// Range bounds validTime (Filter3). The two are ORTHOGONAL: the window [vLo,vHi)
// is IRRELEVANT to the skip. This guard proves it: N=4 staggered files (same
...
func TestDownloadSkipRangeWindowOrthogonalRealLocalFS(t *testing.T) {
	ctx := context.Background()
	lfs := newLocalFS(t)
	alloc := database.NewJemallocAllocator()
	flusher := database.NewL0Flusher(alloc, lfs, "dl-rw-bucket")
	const entity = "alpha"
	...
	// matches sysTime order — the contract sorts by validTimeStart, NOT
	...
	q := func(enable bool) ([]*database.TriTemporalEvent, bool, error) {
		...
		r := database.NewResolver(lfs, lfs, alloc, "dl-rw-bucket", cfg)
	...
	assert.Equalf(t, rangeWinBaseDur, withSkip[0].SystemTime, "the first row is the base+0 file (NOT skipped)")
	assert.Equalf(t, rangeWinBaseDur+1000, withSkip[1].SystemTime, "the second row is the base+1000 file (NOT skipped)")
	t.Logf("skip-range-window-orthogonal REAL *LocalFS: wide window [base+50,base+99999), txTime=base+1500 → %d rows both paths (the base+2000/base+3000 files skipped — their rows fail Filter2; the window is ORTHOGONAL to the sysTime skip)", len(withSkip))
}

// ---------------------------------------------------------------------------
// skip-default-is-on — the opt-OUT contract.
// ---------------------------------------------------------------------------

// TestDownloadSkipDefaultIsOn is the skip-default-is-on guard (the opt-OUT
// contract). §0.c.1 was an opt-IN (MaxRangeRows=0 = UNLIMITED, the
// disclosed unbounded sentinel). INVERTS the default: the skip is ON by
// default (DefaultResolverConfig returns EnableFirstSysSkip=true). The honest
// rationale: the bound is on disk for free (the filename has carried
// FirstSysTimeNs since), the parse is zero-alloc + failsafe, + the
// transitively-safe elimination is a TAUTOLOGY (proven, not heuristic).
...
//
// This guard asserts DefaultResolverConfig().EnableFirstSysSkip == true (the
// production-safe default is ON), AND a literal ResolverConfig{} has
// EnableFirstSysSkip == false (the zero value — the existing..22 guards
// that build `ResolverConfig{MaxL0Files: maxL0}` run with the skip OFF, byte-
// identical to pre-; the existing guards stay GREEN by construction).
func TestDownloadSkipDefaultIsOn(t *testing.T) {
	def := database.DefaultResolverConfig()
	assert.Truef(t, def.EnableFirstSysSkip,
		"DefaultResolverConfig().EnableFirstSysSkip MUST be true (the opt-OUT contract — inverts the earlier opt-IN; the bound is on disk for free, the parse is zero-alloc + failsafe, the §0.e elimination is a tautology)")
	assert.Equalf(t, 1000, def.MaxL0Files, "the other defaults are byte-identical to pre-")
	...
	// The existing..22 guards that build `ResolverConfig{MaxL0Files: maxL0}`
	// run with the skip OFF → byte-identical to pre-(GREEN by construction).
	literal := database.ResolverConfig{MaxL0Files: 1000}
	assert.Falsef(t, literal.EnableFirstSysSkip,
		"a literal ResolverConfig{...} has EnableFirstSysSkip == false (the zero value) — the existing..22 guards run with the skip OFF, byte-identical to pre-")
	t.Logf("skip-default-is-on PASS: DefaultResolverConfig().EnableFirstSysSkip=true (opt-OUT); literal ResolverConfig{} = false (the existing guards stay GREEN by construction)")
}
