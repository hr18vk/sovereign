...
// (ADR-0030) guards (the REAL *LocalFS route guards) — the manifest-
// channel download skip, driven against a REAL *LocalFS (the guard-
// principle "drive the route, not the seam").
//
// (ADR-0029) closed the L0/L1 FILE channel over the REAL *LocalFS (the
// guards in pkg/durability/download_skip_test.go). closes the
...
// the L1's MIN sysTime (the SAME field skips on the file channel). When
// firstSys > the query's txTime (STRICT >), the L1 the manifest points at is
// file-skipped (scan loop) AND every L0 the manifest lists is file-skipped
...
//
// These guards run a REAL Compaction() over the REAL *LocalFS (the production
...
// identity the tautology leverages). The guards query at txTimes below the
...
//
// They reuse the helpers (newLocalFS, insertWindowRow, rangeWinBaseDur,
// rangeOpenEnd) + the countingLocalFS wrapper (the Download counter). The
// seam-level guards (the parser, the EQUIV fuzz, the boundary, the frozen scope,
// the RED controls) live in internal/database; the counter and bridge sister
// guards live in internal/telemetry and pkg/metrics.
//
// §5 CONTRACT: the skip is the transitively-safe elimination on
// the manifest channel. For a query at txTime T + a manifest M with
// filename min(M): min(M) > T ⟹ the L1 M points at is file-skipped AND
...
// ---------------------------------------------------------------------------
// manifest-preserves-answer (REAL *LocalFS) — the headline Law-II preservation.
// ---------------------------------------------------------------------------

// TestManifestPreservesAnswerRealLocalFS is the manifest-preserves-answer
// guard over a REAL *LocalFS. Write N=4 staggered L0 files for E at
...
// path: a skip the seam guard (the internal/database manifest-equiv guard) proved over
...
func TestManifestPreservesAnswerRealLocalFS(t *testing.T) {
	ctx := context.Background()
	lfs := newLocalFS(t)
	alloc := database.NewJemallocAllocator()
	flusher := database.NewL0Flusher(alloc, lfs, "manifest-bucket")
	const entity = "alpha"
	...
	for i := 0; i < 4; i++ {
		...
		insertWindowRow(t, alloc, lfs, flusher, entity, sysNs, rangeWinBaseDur, rangeOpenEnd, []byte("row-"+strconv.Itoa(i)))
	}
	...
	compactor := database.NewL1Compactor(lfs, lfs, lfs, alloc, "manifest-bucket", database.DefaultCompactionConfig())
	...
	q := func(enable bool, txNs int64) (*database.TriTemporalEvent, error) {
		...
		r := database.NewResolver(lfs, lfs, alloc, "manifest-bucket", cfg)
		return r.AsOf(ctx, entity, time.Unix(0, rangeWinBaseDur+50), time.Unix(0, txNs))
	}
	...
	assert.Equalf(t, rangeWinBaseDur+1000, withSkipA.SystemTime,
	...
	const txBelow = rangeWinBaseDur - 500
	...
	t.Logf("manifest-preserves-answer REAL *LocalFS: compaction → L1+manifest(firstSys=base); txTime=base+1500 → dominant base+1000 byte-identical (manifest NOT skipped); txTime=base-500 → manifest+L1+L0s skipped, NotFound byte-identical both paths (Law II preserved)")
}

// ---------------------------------------------------------------------------
// manifest-download-count (REAL *LocalFS) — the disclosure counter + the cut.
// ---------------------------------------------------------------------------

// manifestCountingLocalFS wraps a *LocalFS in a per-key-prefix Download counter
// (manifest vs L1 vs L0) — the instrument the manifest-download-count guard reads to
...
// S3Downloader interface signature), the SAME shape as the
// countingLocalFS wrapper. Single-threaded (one AsOf at a time) → non-atomic
// counts are safe (the countingLocalFS precedent).
...
// TestManifestDownloadCountRealLocalFS is the manifest-download-count guard
// over a REAL *LocalFS. Same N=4 staggered-files + REAL Compaction() setup
// as manifest-preserves-answer. Query at txTime = base-500 (BELOW the manifest's
...
// reads 0 (the L1 download was SKIPPED — file-skip), the L0 Download
// counter reads 0 (all 4 L0s SKIPPED — file-skip), AND the disclosure
...
// downloaded (counter 1) AND the disclosure fired (1) → the guard's
...
// guard's "disclosure fires >= 1" assertion catches it. BOTH the cut AND the
// disclosure are asserted.
func TestManifestDownloadCountRealLocalFS(t *testing.T) {
	ctx := context.Background()
	lfs := newLocalFS(t)
	alloc := database.NewJemallocAllocator()
	flusher := database.NewL0Flusher(alloc, lfs, "manifest-dc-bucket")
	const entity = "alpha"

	// N=4 staggered L0 files (same setup as PRESERVES-ANSWER) + a REAL Compaction().
	for i := 0; i < 4; i++ {
		sysNs := rangeWinBaseDur + int64(i)*1000
		insertWindowRow(t, alloc, lfs, flusher, entity, sysNs, rangeWinBaseDur, rangeOpenEnd, []byte("dc-"+strconv.Itoa(i)))
	}
	compactor := database.NewL1Compactor(lfs, lfs, lfs, alloc, "manifest-dc-bucket", database.DefaultCompactionConfig())
	...
	const txBelow = rangeWinBaseDur - 500 // BELOW the manifest's firstSys==base
	...
	skipR := database.NewResolver(skipLFS, skipLFS, alloc, "manifest-dc-bucket", database.ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: true})
	manBefore := int64(0)
	if telemetry.QueryManifestSkippedFirstSys != nil {
		manBefore = int64(telemetry.QueryManifestSkippedFirstSys.Value())
	}
	_, err = skipR.AsOf(ctx, entity, time.Unix(0, rangeWinBaseDur+50), time.Unix(0, txBelow))
	...
	assert.Equalf(t, 0, skipLFS.l1DLs, "SKIP ON: the L1 download was SKIPPED (file-skip — firstSys==base > base-500)")
	assert.Equalf(t, 0, skipLFS.l0DLs, "SKIP ON: the 4 L0 downloads were SKIPPED (file-skip)")
	...
	// counter fires 0 (the pre-behavior).
	noSkipLFS := &manifestCountingLocalFS{LocalFS: lfs}
	noSkipR := database.NewResolver(noSkipLFS, noSkipLFS, alloc, "manifest-dc-bucket", database.ResolverConfig{MaxL0Files: 1000, EnableFirstSysSkip: false})
	manBefore2 := int64(0)
	if telemetry.QueryManifestSkippedFirstSys != nil {
		manBefore2 = int64(telemetry.QueryManifestSkippedFirstSys.Value())
	}
	_, err = noSkipR.AsOf(ctx, entity, time.Unix(0, rangeWinBaseDur+50), time.Unix(0, txBelow))
	...
	assert.Equalf(t, 1, noSkipLFS.manifestDLs, "SKIP OFF: the manifest IS downloaded (manifestDLs==1 — the pre- behavior; ParseManifest runs)")
	assert.Equalf(t, 1, noSkipLFS.l1DLs, "SKIP OFF: the L1 IS downloaded (l1DLs==1 — no file-skip)")
	...
	// purpose since mark superseded L0s so they are not re-scanned). The
	...
	assert.Equalf(t, 0, noSkipLFS.l0DLs, "SKIP OFF: the 4 L0s are NOT downloaded (l0DLs==0) — the manifest IS downloaded + ParseManifest marks them superseded → removed from tailKeys (the supersession contract; the L0s are in the L1, not re-scanned)")
	assert.Equalf(t, int64(0), manAfter2-manBefore2, "SKIP OFF: the manifest-skip disclosure counter fired 0 (the pre- behavior)")
	t.Logf("manifest-download-count REAL *LocalFS: SKIP ON at base-500 → manifestDLs=%d l1DLs=%d l0DLs=%d disclosure+=%d (the manifest + L1 + L0 downloads CUT); SKIP OFF → manifestDLs=%d l1DLs=%d l0DLs=%d disclosure+=0 (pre-); AsOf byte-identical (NotFound) both paths (Law II)",
		skipLFS.manifestDLs, skipLFS.l1DLs, skipLFS.l0DLs, manAfter-manBefore, noSkipLFS.manifestDLs, noSkipLFS.l1DLs, noSkipLFS.l0DLs)
}

// ---------------------------------------------------------------------------
// manifest-range-window-orthogonal (REAL *LocalFS) — the skip is window-agnostic.
// ---------------------------------------------------------------------------

// TestManifestRangeWindowOrthogonalRealLocalFS is the manifest-range-window-orthogonal
// guard over a REAL *LocalFS. The manifest skip bounds sysTime
// (Filter2); Range bounds validTime (Filter3). The two are ORTHOGONAL: the
// window [vLo,vHi) is IRRELEVANT to the manifest skip. This guard proves it: N=4
...
func TestManifestRangeWindowOrthogonalRealLocalFS(t *testing.T) {
	ctx := context.Background()
	lfs := newLocalFS(t)
	alloc := database.NewJemallocAllocator()
	flusher := database.NewL0Flusher(alloc, lfs, "manifest-rw-bucket")
	const entity = "alpha"
	...
	for i := 0; i < 4; i++ {
		sysNs := rangeWinBaseDur + int64(i)*1000
		insertWindowRow(t, alloc, lfs, flusher, entity, sysNs, sysNs, rangeOpenEnd, []byte("rw-"+strconv.Itoa(i)))
	}
	// REAL Compaction() → 1 manifest (firstSys==base) listing ALL 4 L0s.
	compactor := database.NewL1Compactor(lfs, lfs, lfs, alloc, "manifest-rw-bucket", database.DefaultCompactionConfig())
	res, err := compactor.Compaction(ctx, entity, database.EntityHash8(entity))
	require.NoError(t, err)
	require.Falsef(t, res.AlreadyMoved, "compaction must produce the manifest")
	...
	// the sysTime skip).
	const txBelow = rangeWinBaseDur - 500
	vLo := time.Unix(0, rangeWinBaseDur+50)
	vHi := time.Unix(0, rangeWinBaseDur+99999)
	...
	q := func(enable bool) ([]*database.TriTemporalEvent, bool, error) {
		...
		r := database.NewResolver(lfs, lfs, alloc, "manifest-rw-bucket", cfg)
	...
	t.Logf("manifest-range-window-orthogonal REAL *LocalFS: compaction → manifest(firstSys=base); Range at base-500 → manifest+L1+L0s skipped, window EMPTY both paths (orthogonal); Range at base+1500 → %d rows both paths (manifest NOT skipped — firstSys<=txTime; the window is ORTHOGONAL to the sysTime skip)", len(withSkip2))
}
