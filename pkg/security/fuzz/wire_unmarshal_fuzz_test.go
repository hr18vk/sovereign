...
// wire_unmarshal_fuzz_test.go is FIVE targeted fuzzes, one per
// ingest-path unmarshaler, each asserting the no-panic check INDEPENDENTLY.
// A dispatch fuzz could route a malformed Batch magic to the RELAY arm
...
// (the arm-level coverage the per-arm fuzzes provide).
//
...
// testdata/fuzz/ files stay in lock-step (the SeedCorpusIsValid guard re-derives
...
// property is the round-trip guards's job (TestEnvelope_MarshalRoundTrip et al.),
// NOT this harness's.
//
// Run a single target:
//
...
// convention-frozen NOT md5-pinned) — a panic found here is a fix in this change
// (NOT a frozen-file fix; only crdt.go / crdt_apply.go / the 2 capnp
// schema files are the md5-frozen set).
...
// HIGHEST-YIELD arm per the acceptance gate (the count × bucket unbounded-alloc surface).
...
// disclosed in doc.go + ADR-0038 §6, NOT patched this change. The fuzz exercises
