...
// dispatch_fuzz_test.go is the LOAD-BEARING fuzz target
...
// check — the verdict VALUE is not asserted; the PROCESS STAYS ALIVE).
//
// THE frameSink REACHABILITY (the correction the did not
...
// pkg/security/fuzz (the named home).
//
// UNIT-ISOLATION (the [243c10a] guard principle). nopSink's three
...
// IsXFrame peeks), which is the contract makes falsifiable. Driving the
...
// value is irrelevant to the no-panic check — the fuzz asserts the CALL
...
// ). It feeds DispatchFrame adversarial []byte seeded
...
//
// The -fuzztime NUMBER is the per-change CI gate (the 72-hour is the
// operator's PRODUCTION soak, NOT this change's gate — disclosed in ADR-0038 §6).
func FuzzDispatchFrame(f *testing.F) {
	for _, seed := range dispatchSeeds() {
		f.Add(seed)
	}
	f.Fuzz(func(t *testing.T, wire []byte) {
		// The check: the call RETURNS — verdict value NOT asserted. A panic
