...
// seeds_test.go is the SINGLE SOURCE OF TRUTH for the fuzz seed corpus.
...
// (in seed_corpus_test.go): it re-derives the corpus
...
// guard. The sister guard TestSeedCorpusIsValid enforces a DIFFERENT property
// — no committed seed PANICS the matching unmarshaler (a seed that crashes is
// a corpus defect); it does NOT assert byte-equality (the prior docstring
// over-claimed it did). The two guards are distinct falsifiable properties.
//
// The corpus is the MINIMUM: (a) the valid magic + a well-formed body (the
...
// smallest adversarial shapes). The corpus COUNT is asserted as a NUMBER.
...
// per-process; a fixed seed makes the corpus reproducible —). It is NOT
...
var fuzzSeed = func() maphash.Seed {
	...
	return eng.Uint64ToSeed(0x4441593333000001) // "" + a stable tail
}()
...
// gate stack, per the fuzz's unit-isolation discipline).
