...
// topology.go is the (ADR-0039) TopologyManager — the peer registry
...
// under the seed (— same seed → same Select output EVERY run; the
...
// OFF — byte-identical). It is set on the Gossiper via SetTopology AFTER
...
// happen on the SweepLoop's single goroutine — the SetDigestSink
...
// discipline). Future work that adds live region re-tagging
...
type TopologyManager struct {
	...
	// seed is the per-sweep seed for the inter-region fan-out tie-break (—
	...
	seed uint64
	// fanout is the inter-region fan-out N (the default 3 — the
	...
	// region fan-out — the honest degenerate case; the
	// guard asserts fan-out 0 -> intra-only).
	fanout int
...
// MaxFanout is the upper bound on the inter-region fan-out N. The design's
...
// DefaultRegionFanout is the default inter-region fan-out N. 3
// yields O(log_3 100) ≈ 4-5 rounds to convergence at 100 nodes — the loopback
// gate the guard measures. The operator overrides via
// --region-fanout; the default is the named value.
...
// peer overwrites the tag — the live-re-tagging discipline future work
// extends). Single-writer-before-reader (the SetDigestSink discipline).
...
// design's named value. 0 = intra-region full-mesh only (the honest
// degenerate case).
...
// SetSeed stamps the per-sweep seed for the inter-region fan-out tie-break.
...
func (t *TopologyManager) SetSeed(seed uint64) {
	...
	// the lock anyway for the race-detector's benefit (the guard
...
// self-region (the disclosure counter's gate — the inter-region envelope
...
//
// ctx is accepted for forward-compat (future work that makes Select
...
// the honest target). Future work that wants zero-alloc hoists the
// scratch into the TopologyManager (the zero-alloc ParseManifest
...
func (t *TopologyManager) Select(ctx context.Context) [][16]byte {
	_ = ctx // forward-compat (no I/O this change); documented above.
	t.mu.RLock()
	self := t.selfRegion
	fanout := t.fanout
	seed := t.seed
	...
	// k intra peers — bounded by ≤10/AZ per the ≤45-connections
	// ceiling).
	sortPeerIDs(intra)
	// CRITICAL (determinism): the inter-region fan-out's partial Fisher-Yates
	...
	// selection per call — a violation (a flaky selector). The fix: sort
	...
	// same seed yields the same output EVERY run. The
	...
	// Append the inter-region fan-out (seeded random tie-break —). The
	// fan-out is the default 3 (or the operator's --region-fanout);
...
// size, and the sort is allocation-free (the zero-alloc discipline).
...
// it allocation-free (the zero-alloc discipline).
...
// newSeededRand returns a deterministic rand source under `seed` (— the
// fuzz-seed discipline; math/rand/v2's rand.New(rand.NewPCG(seed, 0)) is
...
// goroutine — the SetDigestSink discipline); the rand is NOT shared
