...
// Package mesh — partition_test.go is the survival gate: a 2-node mesh
// heals a network partition and re-converges to equal MerkleRoot inside a
// <100ms wall-clock SLO, honestly labeled per the honest-labeling rule.
//
// This is the first time the engine PROVES, observably, that a real 2-node mesh
...
// Join — zero data loss by construction). The gate composes the
// PeerSet/Gossiper + the ConvergenceProbe; it does NOT reimplement either.
//
// THE honest-labeling LABEL (load-bearing — gate): the in-process test runs over
...
// (Gate D, 2x c8g, conditional on AWS). Relabeling the in-process number
// as silicon is the mislabeling.
package mesh
...
// TestPartitionHeal_ConvergesUnder100ms is the survival gate (gate C).
// It composes the two-node mesh harness SHAPE (dev CA + two NodeIdentity +
// two tls.Listen on 127.0.0.1:0 + two PeerSets + two Gossipers + the Directory
// pubkey registration) with the ConvergenceProbe to prove a partition
// heals and the mesh re-converges to equal MerkleRoot inside the <100ms SLO.
//
// SEQUENCE (§1 steps 1-7, auditor-traceable):
...
//  7. honesty note: t.Logf records the honest boundary (in-process loopback,
//     NOT silicon intra-AZ latency).
//
// Race-clean (full test runs under -race). The t.Fatalf guards: FAIL if roots
...
func TestPartitionHeal_ConvergesUnder100ms(t *testing.T) {
	// ── 0. Build the two-node mesh (the harness SHAPE, reused) ──
	// Dev CA + two leaves.
	...
	// Per-node gate stack (the wiring, reused).
	...
	// The ConvergenceProbe composes the mesh.
	...
	// A SMALL divergence (a handful of events) — the §1 IS-NOT discipline:
	...
	// territory). The per-delta apply cost (~8ms/envelope over loopback
	...
	t.Logf("step1: after A-only inject — rootA=%x rootB=%x (expect differ)", ra, rb)
	if ra == rb {
		t.Fatalf("step1: roots EQUAL after A-only inject — divergence not created (a setup defect, not a heal defect)")
	}
	...
	ra = engineA.State().MerkleRoot()
	rb = engineB.State().MerkleRoot()
	t.Logf("step2: after baseline sweep — rootA=%x rootB=%x (expect equal)", ra, rb)
	if ra != rb {
		t.Fatalf("step2: baseline convergence FAILED — roots differ (rootA=%x rootB=%x) before any partition; the mesh cannot converge even unpartitioned", ra, rb)
	}

	// ── 3. PARTITION: probe.Partition(B) closes A's conn to B + B's to A ──
	if err := probe.Partition(ctx, identB.NodeID); err != nil {
		t.Fatalf("step3: Partition: %v", err)
	}
	t.Logf("step3: partitioned at %v (bidirectional ClosePeer)", probe.PartitionAt())
	...
	ra = engineA.State().MerkleRoot()
	rb = engineB.State().MerkleRoot()
	t.Logf("step4: after partitioned A-only inject — rootA=%x rootB=%x (expect differ; B lacks the partitioned divergence)", ra, rb)
	if ra == rb {
		t.Fatalf("step4: roots EQUAL while partitioned — B received A's partitioned divergence through a CLOSED conn (the partition leaked; a true defect)")
	}

	// ── 5. HEAL: probe.Heal(B) re-dials BOTH sides; wait for readLoops ──
	if err := probe.Heal(ctx, identB.NodeID); err != nil {
		t.Fatalf("step5: Heal: %v", err)
	}
	t.Logf("step5: healed at %v (re-dial both sides)", probe.HealAt())
	...
	// SILENT TIE-BREAK FRAUD and FAILS gate. The sweep is ALREADY running
	...
	ra = engineA.State().MerkleRoot()
	rb = engineB.State().MerkleRoot()
	t.Logf("step6: healToConv=%v rootA=%x rootB=%x (expect equal)", healToConv, ra, rb)

	// The convergence PROPERTY: roots MUST equal after heal. If they NEVER
	// converge, FAILED (a true defect — the mesh lost data on partition).
	if ra != rb {
		t.Fatalf("step6: roots NEVER converged after heal — rootA=%x rootB=%x. The mesh LOST the partitioned divergence (CRDT Join failed to replicate A's partitioned events to B). FAILED: a mesh that loses data on partition fails the engine's reason to exist.", ra, rb)
	}
	...
	for i := 0; i < partitionedDivergence; i++ {
		...
		if got := engineB.State().Get(eid); len(got) == 0 {
			t.Fatalf("step6: B missing partitioned event %q after heal — zero-data-loss violated (the Join did not replicate the partitioned divergence)", eid)
		}
	}
	t.Logf("step6: zero data loss verified — B holds all %d partitioned events A injected while partitioned", partitionedDivergence)
	...
	// ── 7. honesty note (load-bearing — gate) ──
