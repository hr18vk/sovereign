...
// digest.go is the stratified anti-entropy digest-exchange seam (ADR-0034).
//
...
// This wires the digest exchange, following the SkipList.Seek precedent (a
// dormant primitive whose production consumer was never wired).
//
// THE FIX (the load-bearing correction this change ships). The review ("the
// diff is a strict subset → byte-
...
//
// THE DIGEST EXCHANGE (the two-phase sweep, load-bearing):
//
...
//
// THE WIRE COST (the honest disclosure). The stratified
...
// delta — disclosed honestly in the ADR + the wire-cost guard (the cut is the
// DELTA cut; the wire cost is the digest overhead; the net is the honest
// number the operator reads off sovereign_mesh_* at silicon scale, deferred to
// later silicon work).
//
// The naive "call GenerateDelta(LOCAL IBLT)" is a NO-OP (you diff against
// yourself → empty diff → never converges, the no-op premise). The remote
...
//
// The REJECTED alternative — a /v1/strata-digest control-port route — was
// refuted by direct read of the 2-node test infra (gossip_test.go +
// partition_test.go): both build ONLY PeerSet + Gossiper per node, NO
// ControlServer, NO HTTP listener. The mesh is pure peer-TLS data-plane. A
// control-port route is unreachable from the sweep in the test infra (and is
// the operator-facing surface, not the peer-mesh surface). The peer-TLS
// data-plane digest frame is the ONLY path that works in both the test infra
...
//
// IsBatchFrame(frame) -> recv.HandleBatchFrame (the batch path)
// IsDigestFrame(frame) -> digester.DeliverDigest (the digest path)
// IsHybridFrame(frame) -> recv.HandleHybridFrame (the hybrid path)
// else -> recv.HandleFrame (the frozen relay path)
//
// (ADR-0037) adds the FOURTH arm: a frame tagged WireHybridPQMagic
...
// the mold's "state-carrying frames route to the Receiver" discipline
// forbids).
//
...
// opted in ignores it — the honest cold-start). The fallback counter
// (StratifiedAntiEntropyFallback, the 19th telemetry counter) increments when the
// sweep's digest-exchange phase falls back to oversend (timeout, malformed
// digest, or peel failure) — the honest disclosure.
package mesh
...
// digest-exchange phase (which is blocking on it, per the synchronous
...
// gossip_test.go serveTestConn route identically (the batch path + the
// digest path + the hybrid-PQ batch path + the frozen relay
...
// magic is the highest-rate opt-in path), then IsDigestFrame (the digest
// path), then IsHybridFrame (the hybrid-PQ batch path), then the default
...
func DispatchFrame(frame []byte, peerID [16]byte, recv frameSink, digester DigestSink) receive.AcceptVerdict {
	if attribution.IsBatchFrame(frame) {
		return recv.HandleBatchFrame(frame) // the batch gate stack
	}
	if attribution.IsDigestFrame(frame) {
		if digester != nil {
			digester.DeliverDigest(peerID, frame) // the digest-exchange phase
		}
	...
	if attribution.IsHybridFrame(frame) {
		return recv.HandleHybridFrame(frame) // the hybrid-PQ batch gate stack
	}
...
// POPULATED IBLT `GenerateDelta` subtracts to produce the minimal diff — the
...
// nil IBLT → the sweep's fallback to oversend; a nil SE → dEst=0 → the
// IBLT is sized to the minimum). The channel carries *peerDigest (NOT the bare
// *StrataEstimator the pre-draft carried — that draft had no IBLT on the
...
type peerDigest struct {
	...
	iblt *eng.IBLT            // the remote's FULL digest (the load-bearing field; nil → fallback)
}
...
// caller's DeliverDigest treats a nil IBLT as the fallback to oversend).
// A seLen that overruns the body returns the IBLT slice as nil (a truncated
// SE → the fallback; the IBLT alone is not usable without the SE's seed).
func digestFrameSplit(frame []byte) (seBody, ibltBody []byte) {
	...
	if seStart > len(frame) || seStart+seLen > len(frame) {
		return nil, nil // seLen overruns → malformed (fallback)
	}
...
// other mesh frame is framed). It is the sibling of the batch-frame
...
// channel keys match).
//
// The fix: the frame carries BOTH the SE (the dEst
// sizing hint) AND the full IBLT (the remote's POPULATED digest the receiver's
// `GenerateDelta` subtracts). The pre-draft carried ONLY the SE — the
...
// (the dEst hint) AND the peer's full IBLT (the load-bearing remote digest),
...
// — the honest path, never a silent drop of convergence).
//
...
func (g *Gossiper) DeliverDigest(_ [16]byte, frame []byte) {
	...
	if seBody != nil {
		...
		if err != nil {
			...
			// falls back to oversend rather than blocking forever.
			log.Printf("mesh: malformed strata estimator from %x: %v — sweep will oversend (the digest fallback)", peerID, err)
			select {
			case ch <- pd: // pd.iblt == nil → fallback
			default:
			}
			return
		}
	...
	if ibltBody != nil {
		...
		if err != nil {
			...
			// → fallback; the SE is kept for the dEst log/diagnostic).
			log.Printf("mesh: malformed IBLT digest from %x: %v — sweep will oversend (the digest fallback)", peerID, err)
			select {
			case ch <- pd: // pd.iblt == nil → fallback
			default:
			}
