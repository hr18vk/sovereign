...
// Package mesh — batch_test.go is the batched-transport gate: the
// cross-path MerkleRoot determinism guard (the load-bearing correctness proof
// that batching changes THROUGHPUT, not STATE).
//
// THE guard: a batched path's MerkleRoot MUST EQUAL the per-frame
...
//
// THE honest-labeling LABEL (load-bearing): the in-process test runs over loopback
...
// ns/delta bench is a SEPARATE gate (pkg/receive/batch_bench_test.go).
package mesh
...
// batchMesh is a fully-wired two-node mesh (the harness SHAPE) parameterized
...
// cross-path MerkleRoot determinism guard can compare the two converged roots.
...
// for the given batch size. It mirrors partition_test.go:66-189 (the
// harness SHAPE): dev CA, two NodeIdentity, two engines, Directory pubkey Register
...
// converge to byte-identical state (the cross-path determinism guard).
...
// TestBatchedConverge_100PerBatch is the cross-path determinism gate
// . It proves:
//
...
//
// state; the load-bearing correctness guard: batching changes THROUGHPUT,
//
//	not STATE).
//
// honest-labeling: in-process loopback; the batched ns/delta is a CORRECTNESS +
// through/api claim, NOT a silicon number.
func TestBatchedConverge_100PerBatch(t *testing.T) {
	const total = 1000
	const maxRounds = 20

	// Shared node identities: the cross-path determinism guard requires the
...
func TestBatchRejectTamperedWire(t *testing.T) {
	...
	// guard is the receiver's verify + apply, exercised in-process).
...
func TestBatchRateGateCountsOnce(t *testing.T) {
	...
	//
	// THE guard: each HandleBatchFrame call invokes r.bucket.Accept EXACTLY
	...
	// CORRECTION to this guard's arithmetic: Budget returns 0
	// for a peer that has never been seen (ewma.go:258 — buckets are created LAZILY at
	// :205), so reading it BEFORE the first frame yields 0 and `budgetBefore -
	// budgetAfter` UNDERFLOWS. Pre-that underflow was masked because the
	// over-drain ZEROED the budget and the `budgetAfter != 0` escape hatch below
	// short-circuited the comparison. stops zeroing on an over-budget Drop (a
	// legit origin rejoining after a partition must keep admitting later small
	// advances), which EXPOSED the latent defect. The load-bearing property — "the
	// bucket is decremented ONCE per batch on the originSeq delta, not N times" — is
	// unchanged and is what dropAttempt measures; only this baseline read is fixed.
	// The baseline is therefore taken AFTER the loop's first submission mints the
	// bucket, via budgetAtFirstObservation captured inside the loop.
	var budgetBefore uint64
	dropped := false
	dropAttempt := -1
	var seq uint64
	for attempt := 0; attempt < 100; attempt++ {
		...
		if attempt == 0 {
			// The bucket now EXISTS (the first frame minted it). This is the honest
			// pre-drain baseline; see the correction note above.
	...
	// (step), NOT N×step — the "counts once, not N" guard.
	budgetAfter := bucket.Budget(rateKey[:])
	// The baseline is read AFTER submission 0, so the measured drain covers
	// submissions 1..dropAttempt-1 that were ACCEPTED (the dropping one is not
	// charged — does not charge an unaccepted frame).
	expectedDrain := uint64(dropAttempt-1) * step
