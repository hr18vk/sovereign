...
// whose serial is in the CRL (the ADR-0035 revocation gate).
//
// The load-bearing physics truth (C1, verbatim from the specification §0):
//
...
//	TransmitHeapBuffer live ONLY on the AF_XDP turbo tier (the UMEM
...
//
// ── ADR-0035: the triple hot-reload + the revocation consult ──
//
// earlier the transport reloaded ONLY the leaf (Reload re-parses cert+key);
...
// requires a restart — the CA hot-reload guard).
//
...
// (the revoked-cert rejection guard, the design doc gate byte-proven).
//
...
type TLSConnections struct {
	...
	pqHandshakeReporter func()          //fires on each PQ-KEM handshake (the PQHandshakeNegotiated counter seam); nil = silent
	certPath            string
	keyPath             string
	caPath              string
	crlPath             string
...
// via SetCRLPath — the triple where the CRL may stay unset on a node
...
// string is stable (the guards match on it). The CertRevokedRejected counter
...
//
// the CRL path is NOT set here (NewTLSTransport is the 3-arg
...
// reject) → byte-identical earlier handshake behavior (the off-is-byte-
// identical guard). The CA pool is loaded ONCE here; ReloadCA re-reads it live.
...
// ServerConfig + ClientConfig (the design doc's "mTLS enforcement" — the BOTH-
...
// the M2(a) finding — default Go-tls behavior, NOT new work this change claims).
...
// byte-identical earlier. On a revocation HIT the CertRevokedRejected
...
func (t *TLSConnections) verifyPeerCertificate(rawCerts [][]byte, _ [][]*x509.Certificate) error {
	...
	t.mu.RUnlock()
	if len(revoked) == 0 {
		return nil // no CRL loaded → byte-identical earlier handshake
	}
...
// callback so a revoked CLIENT leaf is rejected at the handshake (the
...
// the 1.2 CipherSuites field). The 1.3-only gate is the guard: Min==Max==1.3
// makes a 1.2/1.1/1.0 negotiation impossible.
//
// ADR-0035: the CA pool is served LIVE via GetConfigForClient. The
...
// listener restart (the CA hot-reload guard). This lifts the earlier
// restriction: Go's tls.Config has NO dynamic-CA hook equivalent to
...
// — ADR-0035 lifts the earlier:126 restriction via a SEPARATE
...
// under the RWMutex. It is the ADR-0035 lift of the earlier:126
...
// validates against the NEW pool (the CA hot-reload guard). The pool swap is
...
// NewTLSTransport has NO CRL path (byte-identical earlier — the
...
// transport rejects nothing — byte-identical earlier). A failed parse
...
// SetRevokedSerialsForTest is the in-process seam the test guards use to arm
// the revocation consult WITHOUT a round-trip through disk (the
// revoked-cert rejection guard mints a leaf, revokes its serial in-process,
// and arms the consult directly — no CRL PEM needed for the unit-level proof;
// the verify_pki_crl harness exercises the FULL disk round-trip). It is the
...
//
// ADR-0036: tls.Dial drives the TLS 1.3 handshake SYNCHRONOUSLY, so
...
// seam a future change may add — disclosed ADR-0036 §6).
func (t *TLSConnections) Dial(network, addr, serverName string) (*tls.Conn, error) {
	...
	t.RecordHandshake(conn.ConnectionState()) //the PQ-KEM disclosure (nil reporter = no-op)
	return conn, nil
}

// ──────────────────────────────────────────────────────────────────────────
// ADR-0035: the automated leaf-rotation TRIGGER (M4).
//
// the design doc gate ("zero-downtime leaf cert rotation every 30
...
// 30 days for the design doc's 30-day cadence; the LIFETIME is the pre-expiry
...
// NEXT handshake presents the NEW serial (the rotation-trigger guard). The trigger
// is OPT-IN (--cert-rotation-enable, default OFF; the opt-IN
...
// the *crypto.MeshCA + wires the minter). A production change that rotates via
...
// load-bearing wiring this change ships.
// ──────────────────────────────────────────────────────────────────────────
...
// transport's cert+key paths; a production change substitutes a KMS/HSM-backed
		...
		for {
			...
			case <-ticker.C:
				...
				if _, rotateErr := minter(); rotateErr != nil {
					...
					// no log surface; a production change wires a logger).
					continue
				}
