...
// architecture PROVES in prose ("epoll +
// SO_ATTACH_REUSEPORT_EBPF vs io_uring: PROVEN"; the matrix row:
// "Socket ingress must utilize edge-triggered epoll combined with native BPF
// steering programs (bpf_sk_select_reuseport)"; and the SOCKHASH
...
// SO_REUSEPORT group sockets — the kernel load is (a FUTURE
...
// and thread migration across a roam). A future change that swaps in a
...
// use a different hash, and the determinism guard is parameterized to
