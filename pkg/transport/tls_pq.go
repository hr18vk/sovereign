...
// ──────────────────────────────────────────────────────────────────────────
// ADR-0036: post-quantum transport readiness — the X25519MLKEM768 KEM
// PROOF surface.
//
...
//	verified ZERO references in pkg/transport earlier). Go 1.24+ advertises
...
//
//	BUG-INJECT control (the proof the guard is load-bearing, NOT a tautology):
...
//
// So does NOT enable the PQ handshake — the engine ALREADY does it via
// the Go default. This change PROVES it (the guard that asserts the negotiated
...
//
// The change sets NO CurvePreferences — keeping the default is the load-bearing
// choice: a forced CurvePreferences=[X25519MLKEM768] would BREAK a peer lacking
// MLKEM (the "opt-IN zero-value = byte-identical" precedent FORBIDS it).
...
//
// The PQ KEM is ORTHOGONAL to the VerifyPeerCertificate callback (the
...
// constant, NOT a CRDT/data-layer change — the read-your-writes seam
// + the mesh sweep + the PKI surface stay byte-identical
// later.
// ──────────────────────────────────────────────────────────────────────────
...
// probe the PQ-KEM-negotiated guard + the runtime /verify harness assert
...
// compares against (the proven-cut mold — the constant is
...
// guards invoke after Handshake() returns. A classical-CurveID handshake (the
