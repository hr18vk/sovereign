...
// LIVE kernel BPF_PROG_TYPE_SK_REUSEPORT + SO_ATTACH_REUSEPORT_EBPF ingress
// fanout, validated on c8g.8xlarge (Graviton4 0xd4f, kernel 6.18, CAP_BPF).
//
// This file is GATED by the `ebpf_kernel` build tag (the same opt-in shape as
// the `pq_preview` build tag). The DEFAULT build (no tag) EXCLUDES it entirely
// — the production ingress seam stays on the in-process ReusePortFanout
// (fanout.go) proven in ADR-0002. This file is the SILICON form of that
...
//
// ANTI-FABRICATION: every Go/kernel symbol called below is cited to a go doc
// or registry source; a symbol without a citation does not belong here.
// golang.org/x/sys@v0.46.0 ships the bpf(2) CONSTANTS but NO Go
// wrappers and NO bpf_attr Go types for the live-program path (grep-verified:
// no BpfProgLoad/BpfMapCreate/BpfProgAttach, no bpf_attr, no
// bpf_sk_reuseport_md in x/sys/unix). There is no `unix.BpfProgLoad` to call.
// The live-program path therefore uses the cilium/ebpf
// loader, which owns the bpf(2) syscall +
// the bpf_attr union + the typed program/map load path, so this file calls
// its typed API, not hand-rolled unix.Syscall bytes against a struct that
// x/sys@v0.46.0 does NOT carry.
//
// cilium/ebpf v0.22.0 is a NEW go.mod dep (the FIRST production-loadable eBPF
// dependency), pinned at the exact version. It is the loader: it owns the
// bpf(2) syscall + bpf_attr union, so this file calls its typed API, not
// hand-rolled unix.Syscall bytes against a struct x/sys@v0.46.0 does NOT
// carry. Every cilium
// symbol called below was grep-verified against the pinned module
...
type KernelFanout struct {
	...
	// DIRECTLY (no in-program hash) — the silicon form of the in-process
	// ReusePortFanout.SelectRoute (cid -> worker index) remap. Populated by
...
// malformed frame is NEVER a drop (the TestEBPFMalformedFrameNoDrop guard).
...
func buildReuseportProgram(mapFD int) *ebpf.ProgramSpec {
	...
	return &ebpf.ProgramSpec{
		...
		// union this change does NOT hand-roll.
		Instructions: insns,
...
// 32-member SO_REUSEPORT socket group the eBPF program selects among, bound to
// the loopback address (127.0.0.1). It delegates to LoadGroupOnIP with a zero
// bindIP (INADDR_ANY) so the loopback and the real-NIC binds share ONE
// socket-creation path, not two. The SO_REUSEPORT setsockopt precedent is
// internal/transport/capnp_server.go:158 (unix.SetsockoptInt(fd,
// unix.SOL_SOCKET, unix.SO_REUSEPORT, 1)); this extends it to the 32-member
// group + the eBPF attach without duplicating its single-socket bind path. The
// constants are doc-cited from x/sys@v0.46.0/unix: SOL_SOCKET
// zerrors_linux_arm64.go:332 = 0x1, SO_REUSEADDR zerrors_linux_arm64.go:386,
// SO_REUSEPORT zerrors_linux_arm64.go:387 = 0xf.
...
// INADDR_ANY (the loopback bind); a non-zero bindIP binds the group to a
// SPECIFIC interface IP — a real-NIC run on a
// non-127.0.0.1 interface (a secondary ENI's private IP, OR a tc-redirect
// target). The SO_REUSEPORT setsockopt precedent is
// internal/transport/capnp_server.go:158 (unix.SetsockoptInt(fd,
// unix.SOL_SOCKET, unix.SO_REUSEPORT, 1)); this change extends it to the
...
func (k *KernelFanout) LoadGroupOnIP(numSockets uint32, port int, bindIP [4]byte) error {
	...
	for i := uint32(0); i < numSockets; i++ {
		...
		// precedent: a zero Addr = INADDR_ANY (the loopback bind); a
		// non-zero Addr binds the group to a specific interface IP (the
		// real-NIC bind).
...
// CID per flow) and the stickiness tests (one pinned CID). Delegates to
...
// then loads + stores the program. The delivery-vs-hash-fallback bench
...
// holds the full churn set. A larger MaxEntries is harmless for the loopback
// tests (it is a cap, not a fixed count).
...
// 4-tuple hash (the SELECT_OR_MIGRATE fallback) — the detached (col-B) path of
// the delivery-vs-hash-fallback bench
// (BenchmarkEBPFDelivery_vs_HashFallback_32c). The program is
// NOT closed (it can be re-attached by AttachProgram); only the group's
// reuse->prog pointer is cleared.
//
// root-cause fix: an earlier DetachProgram (and Close) used
...
// the honest-NEGATIVE guard and the real-NIC bench can compare col A vs
// col B on silicon.
...
func (k *KernelFanout) Close() error {
	...
	// optname — see DetachProgram for the root-cause fix; the earlier
