...
//
// The load-bearing invariant is the copy-before-Pin order (§3 of the
...
// block on ERRQUEUE completion (that is real-epoll-wiring =/12.0).
...
// re-confirmed via an in-module `go build` probe (value 0x4000000).
...
//	(the UMEM ring hands the NIC a userspace address with no
//	sk_buff).
//
...
// this tier is INVISIBLE against verify. Zero-copy is AF_XDP UMEM
...
// fabrication that resets the verdict to NO-GO. See ADR-0006.
