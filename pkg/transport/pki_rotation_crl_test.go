...
// ──────────────────────────────────────────────────────────────────────────
// ADR-0035: the PKI revocation + rotation guards.
//
// the design doc gate ("a node presenting an EXPIRED OR REVOKED cert
// is rejected at the TLS handshake" + "zero-downtime leaf cert rotation every
// 30 days") is a dormant security gate wires. These guards prove it byte-
// for-byte on a real loopback TLS handshake (NOT silicon — the precedent:
// loopback is the unit-level proof, the silicon gear is the scale proof, a
// SEPARATE change). The guards:
//
//	revoked-rejected — a leaf whose serial is in the CRL is REJECTED.
//	sibling-not-revoked — a SIBLING leaf NOT in the CRL PASSES (the CRL
// is serial-scoped, NOT CA-scoped — the M2(b)).
//	expired-rejected — an expired leaf (NotAfter in the past) is
// REJECTED by default Go-tls chain validation
// (the EXPIRED claw — NOT new work this change
// claims; the guard proves the inheritance).
//	CA-hot-reload — ReloadCA swaps the live CA pool (a NEW CA
// signs a leaf the OLD pool rejects + the NEW
// pool accepts after the reload).
//	CRL-hot-reload — ReloadCRL swaps the live revoked-serial set
// (a serial NOT revoked → REJECTED after the
// reload adds it).
//	rotation-trigger — StartRotationManager fires the rotation when
// the live leaf is within the pre-expiry
// lifetime (a sub-second lifetime compresses
// the 30-day cadence into a wall-clock test).
//	off-is-byte-identical — no CRL → the VerifyPeerCertificate callback
// returns nil → byte-identical earlier.
//	leaf-rotation — (TestTLSCertRotation_SIGHUP, EXISTING) the
// SIGHUP seam still swaps the leaf — the
// regression guard (NOT re-run here; the
// existing test covers it).
//	counter-set-21 — Counters() carries the 2 new names
// (CertRotationTriggered + CertRevokedRejected).
//	counters-fire — CertRevokedRejected + CertRotationTriggered
// INCREMENT on the reject / the trigger.
//
// The guards use SetRevokedSerialsForTest (the in-process seam) for the
...
// This change guards can RevokeLeaf + IssueCRL against the SAME CA that signed the
...
// TestPKI_RevokedRejected (the revoked-leaf-rejection guard) proves a leaf whose serial is
// in the CRL is REJECTED at the TLS handshake. The guard uses the FULL disk
...
func TestPKI_RevokedRejected(t *testing.T) {
	...
	if got := atomic.LoadInt32(&rejects); got != 1 {
		t.Fatalf("revoked-rejected: CertRevokedRejected fired %d times, want 1 (the reject MUST fire the counter — the Law V security-gate disclosure)", got)
	}
	t.Logf("revoked-rejected PASS: a leaf whose serial is in the CRL is REJECTED at the handshake + the CertRevokedRejected counter fired (rejects=1)")
}

// TestPKI_SiblingNotRevoked (the sibling-not-revoked guard) proves the CRL is
...
func TestPKI_SiblingNotRevoked(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("sibling-not-revoked: dial (non-revoked sibling leaf should PASS): %v", err)
	}
	defer conn.Close()
	if err := conn.Handshake(); err != nil {
		t.Fatalf("sibling-not-revoked: handshake (non-revoked sibling leaf should PASS): %v", err)
	}
	if err := <-srvErr; err != nil {
		t.Fatalf("sibling-not-revoked: server handshake (non-revoked sibling should PASS): %v", err)
	}
	if got := atomic.LoadInt32(&rejects); got != 0 {
		t.Fatalf("sibling-not-revoked: CertRevokedRejected fired %d times, want 0 (a NON-revoked sibling MUST NOT fire the counter)", got)
	}
	t.Logf("sibling-not-revoked PASS: a sibling leaf NOT in the CRL PASSES the handshake + the counter did NOT fire (the CRL is serial-scoped, NOT CA-scoped)")
}

// TestPKI_ExpiredRejected (the expired-leaf-rejection guard) proves an expired leaf
// (NotAfter in the past) is REJECTED by DEFAULT Go-tls chain validation — the
// EXPIRED claw of the design doc gate is met by the stdlib, NOT by
// new work this change claims (the M2(a) finding). The guard uses
...
func TestPKI_ExpiredRejected(t *testing.T) {
	...
	// this change claims; the callback runs AFTER normal verification, so an
	...
	t.Logf("expired-rejected PASS: an expired leaf (NotAfter in the past) is REJECTED by default Go-tls chain validation (the EXPIRED claw — met by the stdlib, NOT new work this fork claims)")
}

// TestPKI_CAHoReload (the CA-hot-reload guard) proves ReloadCA swaps the live CA
// pool: a leaf signed by a NEW CA is REJECTED by the OLD pool, then ACCEPTED
// after ReloadCA loads the NEW CA. This lifts the earlier
// restriction ("a CA rotation is a trust-root change that requires a
// transport restart").
func TestPKI_CAHoReload(t *testing.T) {
	...
	//the NEW-CA client leaf is REJECTED by the OLD server pool (the
	...
	//overwrite the transport's caPath with the NEW CA PEM + ReloadCA.
	...
	if err != nil {
		t.Fatalf("CA-hot-reload: post-reload dial (the NEW-CA client leaf should PASS after ReloadCA loads the NEW CA into the server's ClientCAs): %v", err)
	}
	defer conn.Close()
	if err := conn.Handshake(); err != nil {
		t.Fatalf("CA-hot-reload: post-reload handshake (the NEW leaf should PASS): %v", err)
	}
	if err := <-srvErr2; err != nil {
		t.Fatalf("CA-hot-reload: post-reload server handshake: %v", err)
	}
	t.Logf("CA-hot-reload PASS: a NEW-CA leaf is REJECTED by the OLD pool, then ACCEPTED after ReloadCA loads the NEW CA (the:126 restriction is lifted)")
}

// TestPKI_CRLHoReload (the CRL-hot-reload guard) proves ReloadCRL swaps the live
...
func TestPKI_CRLHoReload(t *testing.T) {
	...
	//the leaf PASSES (empty CRL).
	srvErr1 := make(chan error, 1)
	go func() { srvErr1 <- handshakeResult(t, ln) }()
	conn, err := tls.Dial("tcp", addr, tr.ClientConfig("node-1"))
	if err != nil {
		t.Fatalf("CRL-hot-reload phase 1 dial (empty CRL, leaf should PASS): %v", err)
	}
	if err := conn.Handshake(); err != nil {
		t.Fatalf("CRL-hot-reload phase 1 handshake (empty CRL, leaf should PASS): %v", err)
	}
	conn.Close()
	if err := <-srvErr1; err != nil {
		t.Fatalf("CRL-hot-reload phase 1 server: %v", err)
	}

	//revoke the leaf's serial + rewrite the CRL at crlPath +
	...
	t.Logf("CRL-hot-reload PASS: a leaf PASSES, then after ReloadCRL adds its serial, the SAME leaf is REJECTED (the live-revocation gate)")
}

// TestPKI_RotationTrigger (the rotation-trigger guard) proves StartRotationManager
// fires the rotation when the live leaf is within the pre-expiry lifetime. The
// guard mints a SHORT-LIVED leaf (1-second validity) + arms the trigger with a
...
// This compresses the design doc's 30-day cadence into a wall-clock test.
func TestPKI_RotationTrigger(t *testing.T) {
	...
	if got := atomic.LoadInt32(&triggers); got < 1 {
		t.Fatalf("rotation-trigger: CertRotationTriggered fired %d times, want >= 1 (the trigger MUST fire when the leaf is within the pre-expiry lifetime)", got)
	}
	t.Logf("rotation-trigger PASS: the rotation trigger fired %d time(s) within the pre-expiry window + the CertRotationTriggered counter fired", triggers)
}

// TestPKI_OffIsByteIdentical (the off-is-byte-identical guard) proves a transport
// with NO CRL (the opt-OUT default) has a VerifyPeerCertificate callback that
// returns nil — byte-identical earlier handshake behavior. The guard
...
func TestPKI_OffIsByteIdentical(t *testing.T) {
	...
	if err := tr.verifyPeerCertificate(nil, nil); err != nil {
		t.Fatalf("off-is-byte-identical: callback(nil) on no-CRL transport returned %v, want nil (the opt-OUT default is byte-identical pre-)", err)
	}
	...
	if err := tr.verifyPeerCertificate(rawCerts, nil); err != nil {
		t.Fatalf("off-is-byte-identical: callback(rawCerts) on no-CRL transport returned %v, want nil (the no-reject fast path)", err)
	}
	...
	conn, err := tls.Dial("tcp", addr, tr.ClientConfig("node-1"))
	if err != nil {
		t.Fatalf("off-is-byte-identical: dial (no-CRL transport should PASS): %v", err)
	}
	defer conn.Close()
	if err := conn.Handshake(); err != nil {
		t.Fatalf("off-is-byte-identical: handshake (no-CRL transport should PASS): %v", err)
	}
	if err := <-srvErr; err != nil {
		t.Fatalf("off-is-byte-identical: server handshake: %v", err)
	}
	t.Logf("off-is-byte-identical PASS: no CRL → callback returns nil → handshake PASSES (byte-identical pre-)")
}

// TestPKI_TelemetryCounterSet21 (the counter-set-21 guard) proves the telemetry Counters() slice carries
// the 2 new names (CertRotationTriggered + CertRevokedRejected). The
// guard is the telemetry counter-grew-honestly proof: the bridge auto-surfaces the 20th +
// 21st series (the §0.f property) IF both are constructed in init() AND
// rebuildCounters() (the fill discipline).
func TestPKI_TelemetryCounterSet21(t *testing.T) {
	cs := telemetry.Counters()
	const wantDistinct = 24 // ADR-0039 RE-PINNED 23 -> 24 (InterRegionEnvelopesShipped — the region-aware gossip inter-region-envelope disclosure); ADR-0037 RE-PINNED 22 -> 23 (HybridFrameAccepted — the hybrid-SIGN-WIRE accept disclosure); ADR-0036 RE-PINNED 21 -> 22 (PQHandshakeNegotiated — the PQ-KEM disclosure); grew 19 -> 21 (CertRotationTriggered + CertRevokedRejected)
	if len(cs) != wantDistinct {
		t.Fatalf("counter-set-21: len(Counters())=%d, want %d (grew the counter 19->21 — TWO counters: CertRotationTriggered + CertRevokedRejected)", len(cs), wantDistinct)
	}
	...
	for name, seen := range wantNames {
		if !seen {
			t.Fatalf("counter-set-21: counter %q MISSING from Counters() (the PKI disclosure counter MUST be in the counter slice)", name)
		}
	}
	t.Logf("counter-set-21 PASS: Counters() carries %d DISTINCT (19->21), both PKI names present (CertRotationTriggered + CertRevokedRejected)", len(cs))
}

// TestPKI_CountersFire (the counters-fire guard) proves CertRevokedRejected +
// CertRotationTriggered INCREMENT on the reject / the trigger. The guard calls
...
// TestPKI_RevokedRejected (above); this guard is the DIRECT increment proof.
func TestPKI_CountersFire(t *testing.T) {
	...
	if afterReject-beforeReject < 1 {
		t.Fatalf("counters-fire: CertRevokedRejected did NOT increment (before=%v after=%v)", beforeReject, afterReject)
	}
	if afterTrigger-beforeTrigger < 1 {
		t.Fatalf("counters-fire: CertRotationTriggered did NOT increment (before=%v after=%v)", beforeTrigger, afterTrigger)
	}
	t.Logf("counters-fire PASS: CertRevokedRejected (%v->%v) + CertRotationTriggered (%v->%v) both INCREMENT", beforeReject, afterReject, beforeTrigger, afterTrigger)
}
...
// errFmt is a small helper to wrap errors with a guard prefix (kept for the
// guards that build error messages; unused imports are avoided by referencing
// fmt in the guards that need it).
var _ = fmt.Sprintf
