...
// ADR-0026 cross-package guards — the T-ARMED gate that FLIPS the
// T6 prose contract "telemetry.Init has ZERO production callers" into a
// GATED assertion of the new reality: telemetry.Init has EXACTLY ONE
...
// this gated guard; the ADR §7 names the specified-flip.
//
...
// TestArmedBridge asserts the new contract: telemetry.Init has
...
// T6 prose contract that was prose-only (the bridge test COMMENT said
// "telemetry.Init has ZERO production callers" but the test body only
// asserted the frozen set). This change DELIBERATELY flips the contract: arming
// Init at boot IS the deliverable — so this gated guard asserts the NEW form
// (ONE production caller, not ZERO) byte-verified via a source grep.
func TestArmedBridge(t *testing.T) {
	caller := countTelemetryInitCallers(t)
	if caller == 0 {
		t.Fatalf("T-ARMED: telemetry.Init has ZERO production callers — the deliverable (arm Init at boot) is NOT shipped. ADR-0026 requires EXACTLY ONE caller under cmd/sovereign-node/")
	}
	if caller > 1 {
	...
	// (the two-exporter separation; T6 carried this as prose,
	// T-ARMED gates it).
	...
	t.Logf("T-ARMED PASS: telemetry.Init has EXACTLY ONE production caller (under cmd/sovereign-node/) and the bridge (pkg/metrics) does NOT call it — the two-exporter separation is intact; the T-Init-uncalled PROSE is REFRESHED to the gated T-ARMED contract")
}
