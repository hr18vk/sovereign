...
// OPERATOR-BLINDNESS defect the engine carried since: the data plane
...
// cadence and tears the delta if OTel fires between scrapes.
...
// Desc per call (that would allocate per scrape). The T3 guard gates
...
// NewExporter, ADR-0023 §3). It does NOT call telemetry.Init — the cmd
// does that (ADR-0026, gated --otel, BEFORE NewTelemetryBridge so the
// bridge captures the post-Init telemetry-counter slice). The bridge reads whatever
...
func (b *TelemetryBridge) Describe(ch chan<- *prometheus.Desc) {
	...
	// telemetry is reflected here WITHOUT a code edit (the telemetry counter property,
...
// INVARIANTS (each load-bearing, gated by a guard):
//
// - CUMULATIVE, NOT DELTA (T1, the §0.b trap guard): counters (modeCounter)
...
// MustNewConstGauge symbol in this version; the specification's shorthand is the
