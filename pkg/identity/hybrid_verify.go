...
// ──────────────────────────────────────────────────────────────────────────
// ADR-0036: the hybrid (Ed25519 + ML-DSA-65) verify — the defense-in-depth
// BOTH-signatures-required seam.
//
// A CRDT delta frame under hybrid signing carries TWO signatures: a hedged
// Ed25519 sig (eddsa_hedge.go:90 SignCRDTFrame, verified by verify.go:74
// VerifyCRDTFrame — the CLASSICAL side) + an ML-DSA-65 sig (pq_mldsa.go:100
// SignCRDTFrame_PostQuantum, verified by pq_mldsa.go:130
// VerifyCRDTFrame_PostQuantum — the PQ side). The hybrid verify checks BOTH;
// EITHER failure rejects the frame.
//
// WHY BOTH, NOT either-or (defense-in-depth):
...
// load-bearing choice; the EITHER-or is the bug-inject control the
// TestPQ_HybridVerifyDual test uses to PROVE the BOTH gate rejects.
//
// The hybrid verify is an opt-in ADD: it does NOT replace the classical
// VerifyCRDTFrame seam (the production default stays classical-only,
// byte-identical). The production wiring is OPT-IN (--hybrid-verify, default
// false — a flag in the node main gates the receive path's call to
// VerifyCRDTFrame_Hybrid vs VerifyCRDTFrame). A classical-only frame (no PQ
// sig) is REJECTED under hybrid-verify — the STRICT mode (a hybrid verifier
// NEVER accepts a single-sig frame; BOTH is the contract).
//
// The PQ SIGN path (SignCRDTFrame_PostQuantum) is benchmarked but the
// production sign seam (the CRDT delta wire) is NOT wired here — the hybrid
// SIGN (a frame carries BOTH sigs) needs the CRDT-delta wire shape changed (a
// future change, disclosed ADR-0036 §6). This file wires the VERIFY; the SIGN
// is a separate later change. The hybrid verify is exercised by
// TestPQ_HybridVerifyDual (which signs under BOTH via the existing
// SignCRDTFrame + SignCRDTFrame_PostQuantum, then verifies + corrupts each
// side).
// ──────────────────────────────────────────────────────────────────────────
...
// circuit AND). The 73.7µs PQ verify (the 4-core bench) runs ONLY after the
// ~60µs classical verify passes — the hybrid cost is the SUM, recorded honestly
// per-op (NOT amortized). On the accept path BOTH verify; on either reject the
// frame is dropped (the receiver's DropVerify verdict).
func VerifyCRDTFrame_Hybrid(edPub ed25519.PublicKey, pqPub *mldsa.PublicKey, msg, edSig, pqSig []byte, ctx string) bool {
	...
	// Ed25519 check + the RejectSmallOrderKey cofactor gate (verify.go:74).
...
// (the ADR-10 CRDTEntry shape; the same 120 the SignCRDTFrame_PostQuantum frame
// [120]byte param takes). The hybrid verify bridges the []byte classical seam
// to the [120]byte PQ seam via this constant; a msg whose length is NOT 120 is
// a frame-shape mismatch the hybrid verify rejects (the classical seam
// tolerates any length, the PQ seam does not — the hybrid honors the STRICTER
// of the two).
