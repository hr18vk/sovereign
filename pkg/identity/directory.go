...
// ErrDirectoryBadPQPubKey is returned by RegisterPQ (ADR-0037) when the
...
// VerifyCRDTFrame. It is the GAP-3 surface this change introduces: the frozen
...
// read-only Lookup.
//
// ADR-0037: the Directory GROWS to carry the origin's ML-DSA-65
...
// backward-compat; the SAME opt-in model hybrid verify uses). The PQ
...
// (the earlier default) LookupBoth returns (edPub, nil, true) — the hybrid
// verify then rejects (the nil-pqPub STRICT mode, the contract carried
...
// is the simpler, deterministic primitive (the pragmatic call).
//
...
type Directory struct {
	...
	// mPQ is the ADR-0037 parallel map carrying the origin's ML-DSA-65
	// public key, keyed by the SAME [16]byte originNodeID as m. It is INDEPENDENT
	// from m so a peer that registered ONLY the classical key (the earlier
	...
	// change; assumes a statically-provisioned directory).
...
// RegisterPQ binds an originNodeID to its ML-DSA-65 public key — the
...
// classical key (Register, the earlier default) leaves mPQ unpopulated for
// that nodeID -> LookupBoth returns (edPub, nil, true) -> the hybrid verify
// rejects (the nil-pqPub STRICT mode, the contract carried forward). A
...
// keys — the ADR-0037 sibling of Lookup, the provisioning seam the
...
// PQ-provisioned"; the hybrid verify rejects a nil pqPub via the
...
// LookupBoth is the NEW seam the hybrid verify path calls.
