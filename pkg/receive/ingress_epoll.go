...
// — the epoll ingress loop WITH KernelFanout wired into the engine's
// receive path (the Gap-1 + Gap-4 closers named in ADR-0003 §8).
//
// The earlier BPF work proved the steering half on loopback, correctness-only:
...
// at GOMAXPROCS=32. EPOLLET is load-bearing: edge-triggered is the
// epoll-routing contract ("epoll COUPLED WITH SO_ATTACH_REUSEPORT_EBPF" — the
...
// DEFEAT the "eradicating cache invalidation" claim.
...
// PROVEN in ADR-0002. Under -tags ebpf_kernel this
// file is the SILICON form: it drives the real 32-member SO_REUSEPORT group the
// existing KernelFanout bound, attaches the eBPF program (KernelFanout.AttachProgram),
...
// CITED here, NOT duplicated. This loop reuses the same x/sys/unix epoll
// surface (EpollCreate1/EpollCtl/EpollWait/EpollEvent) the capnp_server.go loop
// already exercises on this box; it does NOT re-invent bpf(2) (the existing
// cilium loader owns that) and does NOT fork HandleFrame's gate stack.
...
// EpollIngress is the edge-triggered epoll ingress loop that drives the
// existing KernelFanout's SO_REUSEPORT socket group into the engine's receive
// path. It is the Gap-1 + Gap-4 closer: the loop the epoll-routing decision
// names ("epoll
...
// - transport.KernelFanout: the 32-member SO_REUSEPORT group
...
// are the existing KernelFanout surface; this loop does NOT re-load or re-attach — it
// consumes an already-attached KernelFanout (the caller owns the setup, so
// the loop and the test share ONE setup path, not two).
// - *Receiver: the gate-stack composition (HandleFrame). The loop
...
//
// EPOLLET (edge-triggered) is load-bearing: the epoll-routing contract is
...
type EpollIngress struct {
	// fanout is the KernelFanout whose 32 SO_REUSEPORT fds the loop drives.
	...
	// receiver is the gate-stack composition. HandleFrame is called
...
// NewEpollIngress binds an already-attached KernelFanout to a Receiver's
...
// It is the epoll-routing closer: epoll (this loop) COUPLED WITH
// SO_ATTACH_REUSEPORT_EBPF (the attach the caller already did), driving
...
// - unix.Recvfrom(fd, buf, 0) — ebpf_reuseport_test.go:217 (the earlier receipt
// path; the loop reuses the SAME Recvfrom the stickiness table used).
...
func (e *EpollIngress) Serve() error {
	...
	// EPOLLET is load-bearing (the epoll-routing edge-triggered contract).
	for i := 0; i < n; i++ {
...
func (e *EpollIngress) deliverDatagram(datagram []byte) {
	...
	for off < len(datagram) {
		...
		// B6 (ADR-0045): the frame must OWN its bytes.
		// datagram aliases the per-fd Recvfrom buffer (drainFD's e.bufs[bufIdx],
		// reused by the next read), and HandleFrame's relay retainer STORES the
		// slice (: retain takes ownership, no copy) — an alias stored
		// into the relay cache is overwritten by the next datagram. LATENT
		// today (no non-test NewEpollIngress caller); the copy at this seam is
		// the precondition for ever activating. Off the default hot
		// path (this file is ebpf_kernel-tag gated).
		frame := make([]byte, end-start)
		copy(frame, datagram[start:end])
		off = end
