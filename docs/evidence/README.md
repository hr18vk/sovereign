...
| **100-node / 3-region convergence, SLO met** | [convergence-100-node.txt](convergence-100-node.txt) | 100-node mTLS mesh across `us-east-1` / `eu-west-1` / `ap-southeast-2`; convergence timed from origin quiescence |
...
- **`convergence-100-node.txt`** is the multi-region guarantee: all 100 nodes reach
  an identical Merkle root within the 10-second target, with durability fsync-on-write
  armed on every node.
